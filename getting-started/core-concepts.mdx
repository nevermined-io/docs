---
title: "Core Concepts"
description: "Learn how Nevermined adds payments and access control to AI agents, MCP tools, and static assets with payment plans, metering, and per-request entitlement."
icon: "lightbulb"
---
...
## Standards and protocols (x402 and MPP)

Nevermined supports modern payment standards used in agentic commerce. **x402** standardizes how clients and servers negotiate payment-required requests over HTTP headers. Its sibling, the **Merchant Payment Protocol (MPP)**, negotiates the same payment with an RFC 7235 `WWW-Authenticate: Payment` challenge. Both settle against the **same Payment Plans and credits** — the protocol is just the wire handshake.

If you're integrating with x402- or MPP-compatible clients/servers, see the [x402 integration and its MPP sibling](/development-guide/nevermined-x402).
...
<CardGroup cols={2}>
  <Card title="Payment Method" icon="credit-card">
    [**Cards**](/products/payments/overview) (Visa, Stripe, or Braintree) or [**Crypto**](/integrate/patterns/stablecoin-payments) (USDC, EURC on-chain).
  </Card>
...
<Accordion title="Pay-as-you-go">
  ...
  - **Cards**: agents charge via [card delegation](/products/payments/overview)
...
<Steps>
  ...
  <Step title="Users or agents purchase access">
    Purchase a prepaid or pay-as-you-go plan, paid with [card](/products/payments/overview) or crypto.
  </Step>
...
<CardGroup cols={2}>
  <Card title="Cards" icon="credit-card" href="/products/payments/overview">
    Pay with real Visa, Stripe, or Braintree cards via [card delegation](/products/payments/overview). Agents charge against spending delegations with lifetime limits, usage caps, and (for Visa) per-delegation WebAuthn/passkey device binding. No crypto wallet needed.
  </Card>
  <Card title="Crypto" icon="coins" href="/integrate/patterns/stablecoin-payments">
...
| **Set up** | [Enroll a card](/products/payments/card-enrollment) — Stripe, Braintree, or Visa | Fund the account with USDC / EURC |
...
The type comes from the plan's pricing config — `registry.price.isCrypto`, and for fiat plans `metadata.plan.fiatPaymentProvider` names the provider. Rather than read that metadata yourself, let the SDK resolve it. `resolveScheme()` / `resolve_scheme()` fetches the plan (cached for 5 minutes) and returns the scheme:
...
<Note>
  Server-side middleware (Express, FastAPI) and the A2A clients resolve the scheme for you. The direct `getX402AccessToken` / `get_x402_access_token` call does **not** — it defaults to `nvm:erc4337`. For a fiat plan you must pass `scheme: 'nvm:card-delegation'` and have a card enrolled with the matching provider. See [Payment method mismatch](/products/payments/faq#how-do-i-fix-a-payment-method-mismatch) if a payment is rejected.
</Note>
...
Building an **AI agent that needs to pay** for services? Follow [AI Agents: Buy Access Autonomously](/getting-started/ai-agent-purchase) — get an API key, enroll a card, delegate a budget, and make an x402 purchase end to end.
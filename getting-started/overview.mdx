...
- **Sellers can't accept agentic payments** - no metering, no per-request settlement, no programmatic access control
...
<CardGroup cols={2}>
  <Card title="Get started" icon="bolt" href="/integrate/quickstart/5-minute-setup">
    Launch an integration in 5 minutes
  </Card>
  <Card title="API Reference" icon="code" href="/api-reference/introduction">
    Explore the REST API and SDKs
  </Card>
  <Card title="For AI agents" icon="robot" href="/getting-started/ai-agent-purchase">
...
![Diagram: a buyer enrolls a payment method once, then their agent's request to a seller's service is verified and settled by Nevermined — credits charged, seller paid — and the response returns to the buyer's agent.](/images/payment-flow.png)
...
<Steps>
  ...
  <Step title="Create a payment plan">
    - **Payment method** - Fiat ([cards](/products/payments/overview)) or crypto (USDC)
    - **Pricing model** - Fixed per call, or dynamic (seller-defined or cost+margin)
    - **Plan type** - Prepaid (buyers buy a plan upfront) or pay-as-you-go (charge per request)
  </Step>
...
**Sellers** register API endpoints, set pricing per call, and receive payouts. Add a few lines of validation to your server.

**Buyers** purchase a plan (fiat or crypto) or go pay-as-you-go. No human in the loop.

## Use Cases

<CardGroup cols={2}>
  <Card title="Agent Monetization" icon="link" href="/solutions/agent-to-agent-monetization">
    Meter and settle between autonomous agents
  </Card>
  <Card title="Tool Monetization" icon="plug" href="/solutions/mcp-point-of-sales">
    Charge for tools and APIs with per-request billing
  </Card>
  <Card title="Agent Card Payments" icon="credit-card" href="/products/payments/overview">
    Let agents pay with real cards via card delegation
  </Card>
  <Card title="Static Assets" icon="lock" href="/solutions/access-control-monetization-static-resources">
...
| ------------------ | ------------------------------------------------ |
| Payment processing | Fiat ([card delegation](/products/payments/overview)) and crypto (USDC on-chain) |
| Access control     | Gate API access per request via plans and x402   |
| Usage metering     | Per-request tracking and settlement              |
| Settlement         | Automatic payouts to sellers                     |
...
<CardGroup cols={2}>
  ...
  <Card title="SDKs & API" icon="code" href="/api-reference/introduction">

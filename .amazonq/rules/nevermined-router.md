...
- `BCK.ROUTER.0025` (502) — reply too large after a paid request; payment may have gone through. The reply has no `X-Router-Payment-Id`; find the payment by `requestId` in `GET /api/v1/router/payments` and inspect `status` / `merchantSettlementObservedAt`. Do not retry with a fresh `requestId`; reuse the same one if retrying.

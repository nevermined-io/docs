...
- `BCK.ROUTER.0028` (503) — `POST /router/quote` could not price the call because a chain read it depends on failed (the MPP-tempo token `decimals`). A quote never signs or charges anything, so nothing needs unwinding: retry the quote with backoff, and do not route the call meanwhile.

...
<Note>
  The TypeScript example below targets a **crypto** plan. For a **fiat** plan, create the delegation with the card provider (`provider: 'stripe'`, `currency: 'usd'`) and pass `scheme: 'nvm:card-delegation'` to `getX402AccessToken` — or use the `PaymentsClient` (Python tab), which resolves the scheme for you. See [Which payment type does this plan need?](/docs/development-guide/order-plans#which-payment-type-does-this-plan-need).
</Note>

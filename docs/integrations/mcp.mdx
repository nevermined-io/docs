...
[Click here to view the raw context file.](https://nevermined-docs-temp.s3.eu-central-1.amazonaws.com/nevermined_mcp_for_llms.txt) Simply copy its contents and paste them into your LLM's input as a knowledge base.
</Note>
...
Nevermined Payments Library includes **full OAuth 2.1 support** for remote MCP servers, using Nevermined as the authentication provider. All necessary OAuth discovery endpoints are automatically generated and configured, making your server **100% compatible with any MCP client** that supports OAuth authentication.

No manual OAuth server setup is required - simply use the Nevermined MCP integration and authentication is handled automatically.
...
<Tabs>
  <Tab title="TypeScript">
    ...
    payments.mcp.configure({
        ...
        baseUrl: "https://weather-mcp-agent.nevermined.app",
    });
    ```
  </Tab>
  <Tab title="Python">
...
<Tabs>
  <Tab title="TypeScript">
    ```typescript
    // handlers.ts
    import type { PaywallContext } from "@nevermined-io/payments";
    ...
    export async function weatherToolHandler(
      ...
      context?: PaywallContext
        ...
        // You can access the paywall context for observability/logging
        if (context) {
...
<Tabs>
  <Tab title="TypeScript">
    ...
    server.tool(
        "weather.today",
        { title: "Today's Weather", inputSchema: z.object({ city: z.string() }) },
        protectedHandler // Use the wrapped handler
    );
    ```
  </Tab>
  <Tab title="Python">
  ...
  <Tab title="TypeScript">
    ...
    const dynamicCreditsHandler = payments.mcp.withPaywall(
        weatherToolHandler,
        {
            ...
            name: "weather.today",
            credits: (ctx) => {
                // Calculate credits based on city name length
...
<Tabs>
  <Tab title="TypeScript">
    Use `payments.mcp.createApp()` to get a pre-configured Express app with all OAuth endpoints.

    ```typescript
    // server-with-createApp.ts
    import { Payments, EnvironmentName } from "@nevermined-io/payments";
    ...
    // Create Express app with OAuth endpoints
    ...
    app.listen(5001, () => {
        console.log("Server running on https://weather-mcp-agent.nevermined.app");
    });
    ```
  </Tab>
  <Tab title="Python">
...
<Tabs>
  <Tab title="TypeScript">
    ...
    const transport = new StreamableHTTPClientTransport(
        new URL("https://weather-mcp-agent.nevermined.app/mcp"),
        {
            requestInit: {
  ...
  <Tab title="Python">
    ...
    def rpc(method, params):
        r = requests.post(
            "https://weather-mcp-agent.nevermined.app/mcp",

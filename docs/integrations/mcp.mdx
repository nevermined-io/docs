...
const { accessToken } = await paymentsClient.x402.getX402AccessToken(
  ...
  // agentId is optional — informational only
)
```
...
<Note>
For backwards compatibility the server still accepts an `Authorization: Bearer <accessToken>` header as a **deprecated fallback** when `_meta["x402/payment"]` is absent. New integrations should use the in-band `_meta` field.
</Note>
...
// Use tools — for PAID tools, carry the payment in band (see "Client Usage" above):
//   _meta: { "x402/payment": decodeAccessToken(accessToken) }
const result = await client.callTool({
  name: "weather.today",
  arguments: { city: "Madrid" }

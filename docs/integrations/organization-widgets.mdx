...
<Steps>
  ...
  <Step title="Mount the widget in the browser">
    ...
    <Note>
      Every iframe-mounting method also accepts optional `width` / `height` (pixels) for inline sizing — the SDK clamps them to a supported range; omit them for the default overlay. Checkout, enroll, and list also accept `onAuthMismatch` / `onAuthSwitchRequest` for the multi-org identity case — see [Handling identity mismatches](#handling-identity-mismatches).
    </Note>
  </Step>
</Steps>

...
<Steps>
    ...
    <Tabs>
      ...
      <Tab title="Next.js Route Handler">
        ...
        export async function GET() {
          ...
          return NextResponse.json({ session, environment: 'sandbox' })
        }
        ```
      </Tab>
    </Tabs>
  ...
  <Step title="Mount the widget in the browser">
    ...
    Fetch the `session` from your endpoint and pass it straight to the SDK. The same `nvm` instance can mount any of the available widgets.

    ```ts
    import { NeverminedWidgets } from '@nevermined-io/ui-widgets'

    const { session, environment } = await fetch('/api/widget-session').then(r => r.json())

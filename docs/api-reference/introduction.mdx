---
title: "Pre-requisites"
description: "Get your Nevermined API Key to use the Payment libraries and REST API"
icon: "key"
---

To interact with Nevermined programmatically, you need a **Nevermined API Key**. This key is required for both:

- **Payment Libraries** (TypeScript SDK and Python SDK)
- **REST API** (direct HTTP calls)

## Get Your API Key

<Steps>
  <Step title="Sign in to Nevermined App">
    Go to [nevermined.app](https://nevermined.app) and sign in with your account.
  </Step>
  <Step title="Navigate to API Keys">
    Go to **Settings > API Keys** in your dashboard.
  </Step>
  <Step title="Generate a New Key">
    Click **Generate** and copy your new API key. It starts with `nvm:`.

    ```bash
    export NVM_API_KEY="nvm:your-api-key-here"
    ```

  </Step>
</Steps>

<Warning>
  Keep your API key secure. Do not commit it to version control or share it
  publicly.
</Warning>

For a complete walkthrough, see our [5-Minute Setup Guide](/docs/integrate/quickstart/5-minute-setup).

## Choose Your Environment

Nevermined offers two environments:

| Environment | Purpose                 | Network      |
| ----------- | ----------------------- | ------------ |
| **Sandbox** | Testing and development | Base Sepolia |
| **Live**    | Production              | Base Mainnet |

<Tabs>
  <Tab title="TypeScript SDK">
    ```typescript
    import { Payments } from '@nevermined-io/payments'

    const payments = Payments.getInstance({
      nvmApiKey: process.env.NVM_API_KEY,
      environment: 'sandbox' // or 'live'
    })
    ```

  </Tab>
  <Tab title="Python SDK">
    ```python
    from payments_py import Payments, PaymentOptions

    payments = Payments.get_instance(
        PaymentOptions(
            nvm_api_key=os.environ['NVM_API_KEY'],
            environment='sandbox'  # or 'live'
        )
    )
    ```

  </Tab>
  <Tab title="REST API">
    | Environment | Base URL |
    |-------------|----------|
    | **Sandbox** | `https://api.sandbox.nevermined.dev/api/v1` |
    | **Live** | `https://api.live.nevermined.dev/api/v1` |

    ```bash
    curl -X GET "https://api.sandbox.nevermined.dev/api/v1/protocol/agents" \
      -H "Authorization: Bearer $NVM_API_KEY"
    ```

  </Tab>
</Tabs>

## Next Steps

<CardGroup cols={2}>
  <Card
    title="5-Minute Setup"
    icon="rocket"
    href="/docs/integrate/quickstart/5-minute-setup"
  >
    Complete integration walkthrough.
  </Card>
  <Card
    title="REST API"
    icon="code"
    href="/docs/api-reference/protocol/register-agent"
  >
    Make direct HTTP calls to the Nevermined API.
  </Card>
  <Card
    title="TypeScript SDK"
    icon="js"
    href="/docs/api-reference/typescript/installation"
  >
    Install and use the TypeScript Payment library.
  </Card>
  <Card
    title="Python SDK"
    icon="python"
    href="/docs/api-reference/python/installation"
  >
    Install and use the Python Payment library.
  </Card>
</CardGroup>

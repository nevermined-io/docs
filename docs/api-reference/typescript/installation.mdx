---
title: "Installation"
description: "Install and configure the @nevermined-io/payments TypeScript SDK"
icon: "download"
---

## Installation

Install the package via npm or yarn:

```bash
npm install @nevermined-io/payments
```

Or with yarn:

```bash
yarn add @nevermined-io/payments
```

## Basic Setup

```typescript
import { Payments } from '@nevermined-io/payments'

const payments = Payments.getInstance({
  nvmApiKey: process.env.NVM_API_KEY!,
  environment: 'testing' // or 'staging', 'production'
})
```

## Configuration Options

| Option | Type | Required | Description |
|--------|------|----------|-------------|
| `nvmApiKey` | `string` | Yes | Your Nevermined API key |
| `environment` | `Environment` | Yes | `'testing'`, `'staging'`, or `'production'` |
| `appId` | `string` | No | Optional application identifier |
| `version` | `string` | No | Optional version string |

## Environment Details

| Environment | Network | Use Case |
|-------------|---------|----------|
| `testing` | Base Sepolia | Development and testing |
| `staging` | Base Sepolia | Pre-production testing |
| `production` | Base Mainnet | Live applications |

## TypeScript Support

The SDK includes full TypeScript type definitions:

```typescript
import type {
  Payments,
  Agent,
  Plan,
  PriceConfig,
  CreditsConfig,
  ValidationResult
} from '@nevermined-io/payments'
```

## Getting Your API Key

1. Go to [nevermined.app](https://nevermined.app)
2. Sign in with Web3Auth
3. Navigate to **Settings > API Keys**
4. Generate and copy your key

```bash
export NVM_API_KEY="nvm:your-api-key-here"
```

## Next Steps

<CardGroup cols={2}>
  <Card title="Payments Class" icon="code" href="/docs/api-reference/typescript/payments-class">
    Core SDK class and methods
  </Card>
  <Card title="Quick Start" icon="rocket" href="/docs/integrate/quickstart/typescript">
    Get started quickly
  </Card>
</CardGroup>

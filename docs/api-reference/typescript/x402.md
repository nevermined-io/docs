...
|--------|---------|----------|------------|
| `nvm:erc4337` | `eip155:84532` (or other CAIP-2 chain ID) | Crypto payments | ERC-4337 UserOps + session keys |
| `nvm:card-delegation` | `stripe` or `braintree` | Fiat/credit card | Stripe PaymentIntent or Braintree `transaction.sale` + credit burn |

The scheme is determined by the plan's pricing configuration. Plans with `isCrypto: false` use `nvm:card-delegation`; all others use `nvm:erc4337`. For card-delegation plans, the network is derived from the plan's `fiatPaymentProvider` metadata (`'stripe'` or `'braintree'`). The SDK auto-detects both via `resolveScheme()` / `resolveNetwork()`.
...
| `providerPaymentMethodId` | `string` | No | PSP-side payment method handle: Stripe `pm_...` ID, or Braintree `paymentMethodToken` |
...
| `merchantAccountId` | `string` | No | Merchant account identifier on the PSP: Stripe Connected Account ID, or seller's per-currency Braintree `merchantAccountId` |
...
| `provider` | `string` | Payment provider (`stripe` or `braintree`) |
...
- **accepted**: Payment method specification
  ...
  - **network**: `eip155:84532` (Base Sepolia) for crypto, `stripe` or `braintree` for fiat (matches the plan's `fiatPaymentProvider` metadata)
...
When `scheme` is set to `'nvm:card-delegation'`, the `network` is auto-resolved from the plan's `fiatPaymentProvider` metadata — either `'stripe'` or `'braintree'`. Pass `network` explicitly to override.

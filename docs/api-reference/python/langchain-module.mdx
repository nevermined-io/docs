...
description: "Decorator, helpers, and exceptions for protecting LangChain and LangGraph tools with Nevermined x402 payments"
icon: "link-simple"
---

The `payments_py.x402.langchain` module provides the building blocks for monetizing LangChain tools and LangGraph agents with the x402 protocol. It ships in the `[langchain]` optional extra.
...
<Note>
  Looking for the conceptual walk-through? See the **[LangChain integration guide](/docs/integrate/add-to-your-agent/langchain)**. For a runnable end-to-end demo, see the [`langchain-paid-agent-py` tutorial](https://github.com/nevermined-io/tutorials/tree/main/langchain-paid-agent-py).
</Note>
...
| Symbol | Purpose |
| ------ | ------- |
| [`requires_payment`](#requires-payment) | Decorator that gates a `@tool` on an x402 access token |
| [`PaymentRequiredError`](#paymentrequirederror) | Exception raised when the token is missing or verification fails |
| [`last_settlement`](#last-settlement) | Read the most recent settlement receipt from outside the agent's call stack |
| [`create_paid_react_agent`](#create-paid-react-agent) | LangGraph ReAct agent builder that lets `PaymentRequiredError` propagate intact |
...
- **Fixed plans** (`plan.credits.minAmount == plan.credits.maxAmount`) always burn `plan.credits.maxAmount`. The decorator's `credits=N` is then effectively a no-op (per [nvm-monorepo#1568](https://github.com/nevermined-io/nvm-monorepo/issues/1568)).
...
Raised by `@requires_payment` when the token is missing from `config["configurable"]["payment_token"]`, or when the facilitator rejects the token (expired, invalid signature, insufficient balance, …). Carries the full `X402PaymentRequired` payload so callers can run the x402 discovery flow.
...
try:
    ...
    # accepts.network   → CAIP-2 chain or provider name (stripe, braintree, …)
...
- [x402 Protocol module](/docs/api-reference/python/x402-module) — token generation, delegation config, scheme resolution.

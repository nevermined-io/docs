...
payment_required_fiat = X402PaymentRequired(
    ...
    accepts=[
        X402Scheme(
            scheme="nvm:card-delegation",
            network="stripe",  # or "braintree" — must match the plan's fiatPaymentProvider
            plan_id="your-plan-id",
            extra=X402SchemeExtra(
                http_verb="POST",
                agent_id="agent-123"
...
payment_required = build_payment_required(
    ...
    agent_id="agent-123",
    http_verb="POST",
    scheme="nvm:card-delegation"  # Force fiat scheme; network auto-resolved from plan metadata (stripe or braintree)
)

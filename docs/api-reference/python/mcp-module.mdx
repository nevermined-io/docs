...
async def main():
    result = await payments.mcp.start({
        "port": 5001,
        "planId": "plan-123",            # required
        "serverName": "code-assistant-mcp",
        "agentId": "agent-123",          # optional — informational only
        "version": "1.0.0"
    })
...
| `planId` | `str` | Yes | Nevermined plan ID to charge against |
| `serverName` | `str` | Yes | Human-readable name |
| `agentId` | `str` | No | Nevermined agent DID — optional, informational only (used as the OAuth `client_id` when present; otherwise one is generated) |

...
- My builder wallet (receives payments): 0xYOUR_WALLET
- Environment: sandbox (NVM_API_KEY is set)

Use the SDK price/credits helpers to build the config, register the agent and plan together, then print the agentId and planId and confirm both are published. Reference https://nevermined.ai/docs/development-guide if you need the method signatures.
```
  ...
  <Step title="Build the price and credits config">
    ...
    <Tabs>
      ...
      <Tab title="Python">
        ...
        price_config = get_fiat_price_config(10_000_000, BUILDER_ADDRESS, "USD")
        # or crypto: get_erc20_price_config(10_000_000, USDC_ADDRESS, BUILDER_ADDRESS)

...
<Tabs>
  ...
  <Tab title="Python">
    ...
    # EUR fiat plan (priced in euro cents)
    eur_price_config = get_fiat_price_config(2900, builder_address, Currency.EUR)
...
If you want the Nevermined platform to enforce a route-level allowlist on top of your library middleware, provide `endpoints` (and optionally `openEndpoints` for public routes). When set, only requests matching one of the entries are accepted by x402 verify; mismatches reject with `BCK.PROTOCOL.0031`. You can also publish a discoverable agent definition by setting `agentDefinitionUrl`.

...
- [`BCK.APIKEY`](#bck-apikey) — 15 codes
...
- [`BCK.DELEGATION`](#bck-delegation) — 6 codes
...
| Code | HTTP | Category | Retryable | Message | Hint |
|---|---|---|---|---|---|
| <a id="bck-version-0001"></a>`BCK.VERSION.0001` | 400 | validation | ❌ | Unknown or unsupported API version | Send Nevermined-Version as MAJOR.MINOR (e.g. "1.0") within the supported range. GET /api/v1/meta/versions lists the supported range; see https://docs.nevermined.app/api-reference/versioning. |
...
| <a id="bck-vgs-0002"></a>`BCK.VGS.0002` | 502 | integration | ❌ | VGS network-token provision-on-create failed (best-effort) | #2057 item 3: provisioning a portable network token for a freshly enrolled unified VGS card failed. BEST-EFFORT and never surfaced to the client — enrolment completes, the credential persists with networkToken NULL, and the charge path falls back to the instrument rail. The unified-rail twin of BCK.VISA.0021 (Visa-agentic rail): split out so an alert can pinpoint which rail dropped a token. Inspect cardId/credentialId in the log context. |

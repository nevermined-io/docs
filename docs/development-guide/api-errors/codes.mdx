...
- [`BCK.AUTH`](#bck-auth) — 11 codes
- [`BCK.BILLING`](#bck-billing) — 16 codes
- [`BCK.BRAINTREE`](#bck-braintree) — 10 codes
- [`BCK.COMMON`](#bck-common) — 33 codes
- [`BCK.CREDITS`](#bck-credits) — 6 codes
- [`BCK.DELEGATION`](#bck-delegation) — 2 codes
- [`BCK.ENTITLEMENTS`](#bck-entitlements) — 1 code
...
- [`BCK.ORGANIZATIONS`](#bck-organizations) — 30 codes
...
- [`BCK.PROTOCOL`](#bck-protocol) — 49 codes
...
- [`BCK.WIDGET_SESSION`](#bck-widget_session) — 16 codes
- [`BCK.X402`](#bck-x402) — 18 codes

## `BCK.ACCOUNT_PROVISIONING` { #bck-account_provisioning }
...
## `BCK.ENTITLEMENTS` { #bck-entitlements }
...
## `BCK.INVITATIONS` { #bck-invitations }
...
| <a id="bck-organizations-0019"></a>`BCK.ORGANIZATIONS.0019` | 403 | business | ❌ | Customer management is not available for this organization tier | Customer tracking is a Premium+ feature. Upgrade the organization to Premium or Enterprise to access the customers tab and per-customer activity. |
...
## `BCK.ORG_INTEGRATION` { #bck-org_integration }
...
| <a id="bck-protocol-0011"></a>`BCK.PROTOCOL.0011` | 403 | integration | — | Insufficient balance | Top up the buyer balance (USDC/EURC/USD/EUR depending on plan currency) before retrying. The protocol reverted because the order amount exceeds the wallet balance. |
...
| Code | HTTP | Category | Retryable | Message | Hint |
|---|---|---|---|---|---|
| <a id="bck-widget_keys-0001"></a>`BCK.WIDGET_KEYS.0001` | 500 | internal | ❌ | Widget key generation failed | Inspect the underlying error in logs. Widget-key minting failed — usually a JWT signing or DB-write error. |
...
| Code | HTTP | Category | Retryable | Message | Hint |
|---|---|---|---|---|---|
| <a id="bck-widget_session-0001"></a>`BCK.WIDGET_SESSION.0001` | 401 | auth | — | Invalid or expired widget init token | Re-issue the init token from the widget host. Init tokens are short-lived; refresh just before launching the widget. |
| <a id="bck-widget_session-0002"></a>`BCK.WIDGET_SESSION.0002` | 401 | auth | — | Organization has no active widget key | Generate a widget key in the org admin UI before embedding the widget. Without an active key the org cannot accept widget sessions. |
...
| <a id="bck-widget_session-0006"></a>`BCK.WIDGET_SESSION.0006` | 500 | auth | — | Widget encryption key not configured or invalid | Configure `WIDGET_ENCRYPTION_KEY` as a valid 32-byte hex value. The widget host uses this key to seal session secrets. |
| <a id="bck-widget_session-0007"></a>`BCK.WIDGET_SESSION.0007` | 401 | auth | — | Widget init token is missing required fields | Reissue the init token from the widget host with the documented claim set (org, widget, expiry). |
...
| <a id="bck-widget_session-0013"></a>`BCK.WIDGET_SESSION.0013` | 401 | auth | — | Widget init token is missing the required email claim | Reissue the init token with the email claim populated. Email is required for widget-session attribution. |
| <a id="bck-widget_session-0014"></a>`BCK.WIDGET_SESSION.0014` | 403 | auth | — | Host identity email does not match widget init token email | Sign in to the host application with the email address the widget init token was bound to. Cross-email host sessions are rejected to prevent identity-spoofing across embedded widgets. |
| <a id="bck-widget_session-0015"></a>`BCK.WIDGET_SESSION.0015` | 401 | auth | — | Host identity verification is missing the privy-id-token header | Forward the Privy identity token from the host browser session as the privy-id-token request header alongside the widget init call. |
| <a id="bck-widget_session-0016"></a>`BCK.WIDGET_SESSION.0016` | 401 | auth | — | Host identity verification rejected by Privy | Refresh the Privy identity token in the host application; expired or revoked tokens land here. If it persists, inspect the Privy dashboard for the user. |

...
- An unknown or out-of-range value is rejected with `400` and the stable error code `BCK.VERSION.0001`.
...
const payments = Payments.getInstance({
  ...
  environment: 'live',
  // Optional: pin a different backend API version (defaults to LOCKED_API_VERSION)
  version: '1.0',
})
```
...
payments = Payments(
    PaymentOptions(
        ...
        environment="live",
        # Optional: pin a different backend API version
        api_version="1.0",
    )
)
```

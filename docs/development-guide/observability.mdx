...
llm = ChatOpenAI(**langchain_config)
```
  </Tab>
</Tabs>
...
- **Batch Requests**: Process multiple requests together for efficiency
...
async def ask(
    ...
    try:
        ...
        if not balance_info.get("is_subscriber") or balance_info.get("balance", 0) < 1:

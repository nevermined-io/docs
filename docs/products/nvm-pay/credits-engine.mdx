...
const dynamicCredits = getDynamicCreditsConfig(
  ...
  100n                // Maximum credits per request
)
...
const expiringCredits = getExpirableDurationConfig(ONE_MONTH_DURATION)
// Note: For time-based plans, credits per request is always 1
```
...
Prevent micro-transaction failures:

```typescript
import { getDynamicCreditsConfig } from '@nevermined-io/payments'

const minThresholdConfig = getDynamicCreditsConfig(
  1000n,              // Total credits
  10n,                // Minimum credits per request
  100n                // Maximum credits per request
)
```

...
| Network | CAIP-2 blockchain (e.g., `eip155:84532`) | `visa:vts` |
...
This extension is identified by the scheme `visa` and network `visa:vts`.
...
| `network` | `string` | Yes | Payment network. MUST be `"visa:vts"`. |
...
| Network | `accepted.network` MUST equal `"visa:vts"` |
...
| `network` | `string` | Network identifier (`"visa:vts"`). |

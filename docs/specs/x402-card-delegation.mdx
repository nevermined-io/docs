...
sequenceDiagram
  ...
  rect rgb(255, 240, 255)
    note over server,stripe: Phase 3: Settlement (Credit-Based)
    ...
    alt Insufficient credits
      facilitator->>stripe: Create PaymentIntent (plan price)
      alt Stripe failure
        stripe-->>facilitator: Payment failed
...
5. If balance is insufficient for `maxAmount`:
   ...
   c. Create off-session Stripe PaymentIntent for the plan price
   d. On Stripe failure: roll back spend counter, return error
...
| `orderTx` | `string` | Stripe PaymentIntent ID if auto top-up occurred (only present when card was charged). |

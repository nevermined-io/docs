...
sequenceDiagram
  ...
  rect rgb(255, 248, 240)
    ...
    facilitator-->>client: Delegation JWT token
  end
  ...
  rect rgb(255, 240, 255)
    ...
    alt Stripe failure
      ...
      facilitator->>facilitator: Check if limits reached → set Exhausted
...
**Step 9.** Client requests a delegation from the facilitator, specifying the scheme, spending limits, and optional constraints.
...
**Step 11.** Facilitator signs a JWT token containing the delegation claims (see Section 3.3) and generates the x402 access token.

**Step 12.** Facilitator returns the base64-encoded x402 access token to the client. This token is what the client will include in the `PAYMENT-SIGNATURE` header when accessing protected resources.
...
**Step 16.** The Client selects the `nvm:card-delegation` scheme from `accepts[]` and sends the delegation JWT as the x402 access token.

<Note>
For the `nvm:card-delegation` scheme, the `x402AccessToken` is the **raw delegation JWT** (not wrapped inside a PaymentPayload object). This differs from the `nvm:erc4337` scheme where the token is a base64-encoded PaymentPayload containing session keys. The facilitator auto-detects the scheme by inspecting the token format (JWT vs base64 PaymentPayload).
</Note>

**Step 17.** Client sends an HTTP request to the server including the delegation JWT in the `PAYMENT-SIGNATURE` HTTP header (base64-encoded).

```http
GET /api/resource HTTP/1.1
Host: agent.example.com
PAYMENT-SIGNATURE: <base64-encoded-delegation-jwt>
```
...
{
  "paymentRequired": { ... },
  "x402AccessToken": "<delegation-jwt>",
  "maxAmount": "500"
}
```

**Step 19.** Facilitator performs the following verification checks:
...
| `PAYMENT-SIGNATURE` | Client -> Server | Base64-encoded delegation JWT (for `nvm:card-delegation`) or base64-encoded `PaymentPayload` (for `nvm:erc4337`) |
| `PAYMENT-RESPONSE` | Server -> Client (200) | Base64-encoded settlement receipt |

<Note>
For the `nvm:card-delegation` scheme, the `PAYMENT-SIGNATURE` header carries the base64-encoded **delegation JWT directly**. This is different from the `nvm:erc4337` scheme, which wraps its data in a `PaymentPayload` object before base64-encoding. The facilitator auto-detects the scheme by inspecting the decoded token format.
</Note>

...
- **Fiat delegations** (`provider: 'stripe' | 'braintree'`): Use the configured Payment Service Provider (Stripe PaymentIntents or Braintree `transaction.sale`) backed by pre-authorized card delegations for automatic card-funded credit top-ups when the subscriber's balance is insufficient.

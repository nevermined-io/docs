...
sequenceDiagram
  ...
  rect rgb(240, 255, 240)
    ...
    client->>server: Request + PAYMENT-SIGNATURE header (base64-encoded PaymentPayload containing JWT)
...
**Step 16.** The Client selects the `nvm:card-delegation` scheme from `accepts[]` and constructs a PaymentPayload containing its delegation JWT token.

**Step 17.** Client sends an HTTP request to the server including the x402 Payment Payload in the `PAYMENT-SIGNATURE` HTTP header.
...
| `PAYMENT-SIGNATURE` | Client -> Server | Base64-encoded `PaymentPayload` object (containing JWT) |

...
description: "Let users enrol a credit or debit card once and delegate a spending budget to AI agents — directly in the app, via a white-labeled redirect, or embedded as a widget."
icon: "user-shield"
---

Credit-card delegation is how a human user authorises an AI agent to spend on their behalf, within an explicit budget and expiry, without handing over the card itself. The card is tokenized once via Stripe, Braintree, or Visa Intelligent Commerce, and the delegation defines exactly how much the agent can charge, for how long, and against which API key.
...
<CardGroup cols={3}>
  <Card title="Directly in the App" icon="browser" href="#1-directly-in-the-nevermined-app">
    Users go to **nevermined.app**, sign in, enrol a card, and create a delegation. Zero integration work.
  </Card>
...
<Note>
Options **2** and **3** are organisation-scoped — they require an active [Nevermined organisation](/docs/integrations/organizations) with at least one member. Option **1** works for any signed-in user, with or without an organisation.
</Note>
...
<Steps>
  ...
  <Step title="Open Payment Methods">
    From the sidebar, navigate to **Payment Methods**. This is the dashboard for every card you have enrolled and every active delegation.
  </Step>
...
For tools that run **outside the browser** — CLIs, terminal-based agents, desktop apps — but still need to authorise a card. The pattern mirrors `nvm login`: print a URL, the user clicks it, the browser handles the chromeless card setup, and your process receives the result at a localhost callback.

<Warning>
This path requires an **active Nevermined organisation** with at least one member. The user driving the flow must be a member of that organisation. Card setup is positioned as an organisation feature — see [Platform Partners](/docs/integrations/organizations) to upgrade.
</Warning>
...
<Steps>
  ...
  <Step title="Mint a widget session">
    Authenticate the request with the user's own NVM API key (the one they get from `nvm login`). The endpoint rejects callers who are not members of the requested organisation.
    ...
    | 403 | `BCK.WIDGET_SESSION.0018` | The caller has no active organisation memberships. Create or join one first. |
...
- **Self-mint** sessions (the `POST /widgets/session/self` path documented above) reject any non-localhost host. Use the widget-key flow (see option 3) if you need remote redirects.
...
Your end-users never leave your domain. You mint a session token on your server with the organisation's widget-key, the browser exchanges it for an iframe session, and the user completes the card-enrollment + delegation flow inside your own page.

<Warning>
Like option 2, the iframe widget flow requires an **active Nevermined organisation**. The widget-key is the credential that scopes every iframe session to your org's users — see [Platform Partners](/docs/integrations/organizations) for the upgrade path.
</Warning>
...
- You need brand consistency — the iframe inherits the parent page's framing and your organisation's branding theme.
...
| **Requires an organisation** | No | Yes | Yes |
...
- [Platform Partners](/docs/integrations/organizations) — how to become an organisation

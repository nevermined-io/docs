...
<AccordionGroup>
  ...
  <Accordion title="Who is the merchant of record?">
    **You are on every rail** — though for different reasons:

    * **Stripe**, and **Visa Trusted Agent Protocol** (which settles through
      Stripe Connect): Nevermined creates the charge with Stripe's
      `on_behalf_of` set to your connected account, which makes your account
      the merchant of record. Your statement descriptor appears on your
      buyers' card statements, and the charge settles in your country and
      currency. Nevermined's fee is deducted as a Stripe application fee on
      the charge.
    * **Braintree**: the charge runs against your own OAuth-connected
      gateway, so it settles on your Braintree account directly.
  </Accordion>
  <Accordion title="How are chargebacks handled?">
    It depends on the rail, because the charge lands in a different place:

    * **Stripe**, and **Visa Trusted Agent Protocol**: Stripe debits
      **Nevermined's** balance for disputes on these charges, so the
      chargeback exposure sits with us rather than with you.
    * **Braintree**: the charge settles on your own connected gateway, so
      **you** absorb the chargeback.

    On every rail we surface the chargeback or charge reference so you can
...
| **Enterprise** | **\$500 / month** | Premium + unlimited agents, plans, and seats, plus the [customer CRM](/docs/solutions/organizations/customers) and the realtime activity stream (WebSocket). |

The live price for each tier is shown on the upgrade screen, and a subscription covers **one environment only** — running both sandbox and live means subscribing to each. See [Plans & Billing](/docs/solutions/organizations/plans-and-billing) for both.

The caps count what *you* publish — agents are your registered services, plans are your pricing packages. The number of customers buying them is unlimited on every tier. See [Tiers at a glance](/docs/solutions/organizations/overview#tiers-at-a-glance) for the full comparison.

...
<Tabs>
  <Tab title="TypeScript">
    ...
    app.post('/purchase-key', async (req, res) => {
      ...
      // On a PAYG plan, maxAmount is inert at settle: each call to
      // /purchase-key triggers one card charge for the plan price
      // regardless of the value. On a credits-bundle plan, maxAmount
      // is the per-request burn amount (in credits, not cents).
  ...
  <Tab title="Python">
    ...
    async def purchase_key(request: Request):
        ...
        # On a PAYG plan, max_amount is inert at settle: each call to
        # /purchase-key triggers one card charge for the plan price
        # regardless of the value. On a credits-bundle plan, max_amount
        # is the per-request burn amount (in credits, not cents).
...
<Tabs>
  <Tab title="Organization path">
    ...
    Swap `api.live` for `api.sandbox` for the sandbox environment. Copy the rendered output to your own domain (e.g. `your-api.com/llms.txt` and `your-api.com/integrations/nevermined.md`, or `your-api.com/docs/integrations/nevermined.md` if your docs live under a subpath). The org endpoint gives you the canonical format; you control where agents discover it. Updates are picked up on next render (5-minute cache).
  </Tab>
  <Tab title="DIY path">

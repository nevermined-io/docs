...
<Warning>
  **Production handlers must set both response headers.** The handler above
  omits the `payment-required` (on 402) and `payment-response` (on 200)
  headers for brevity. Per the
  [card-delegation spec](/docs/specs/x402-card-delegation) §4.3, both are
  required so agents using the buyer-side middleware can discover payment
  requirements and confirm settlement. The optional middleware below sets

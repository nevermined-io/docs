...
<AccordionGroup>
  ...
  <Accordion title="Who is the merchant of record?">
    **You are.** Nevermined creates each card charge with Stripe's
    ...
    currency. Nevermined takes its fee as a Stripe application fee rather
    than by holding your revenue.
  </Accordion>
...
<Warning>
  **A subscription covers one environment only.** Upgrading your **sandbox**
  ...
  separately on live. See [Plans & Billing](/docs/solutions/organizations/plans-and-billing).
</Warning>

Prices above are indicative; the live price for each tier is shown on the upgrade screen. Settlement fees on agent payments are documented in the [Payments overview](/docs/products/payments/overview).

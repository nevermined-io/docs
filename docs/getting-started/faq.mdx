...
<Accordion title="What is NVM Pay?">
  NVM Pay lets users enroll a Visa, Stripe, or Braintree card, set spending limits, and let agents charge it directly via x402. No checkout pages, no human in the loop. Manage everything at [pay.nevermined.app](https://pay.nevermined.app).
</Accordion>
...
<Accordion title="How do I revoke an agent's ability to spend?">
  One click in the [dashboard](https://pay.nevermined.app) or `DELETE /api/v1/delegation/{delegationId}`. Revocation is immediate: no grace period, no cached tokens. The agent loses payment capability the instant you revoke.
</Accordion>

<Accordion title="Do I need a crypto wallet?">
  No. NVM Pay works with regular Visa, Mastercard, Amex, and Discover cards through Visa, Stripe, or Braintree. All payments are settled via card rails. No wallet or stablecoins required.
</Accordion>
...
<Accordion title="How does my agent know which delegation to use?">
  ...
  See [Delegation Selection](/docs/products/nvm-pay/mandate-selection) for details.
</Accordion>
...
<Accordion title="What's the difference between Stripe Connect and NVM Pay?">
  ...
  **NVM Pay** is for *users* who want agents to pay with their card. The user enrolls a card, creates a spending delegation, and agents charge against it via x402.

...
In the Nevermined ecosystem, a _service_ is anything you expose for paid access. That might be:
...
<Steps>
  <Step title="Register the service/resource">
    Define your service interface (API, MCP tool, or protected asset) and
    metadata.
  </Step>
  <Step title="Create plans">
    Create one or more payment plans: pricing, credits, duration, and
    consumption rules.
  </Step>
  ...
  <Step title="Validate before delivery">
    Validate entitlement (tokens / headers / proxy validation) before serving
    requests or granting access.
  </Step>

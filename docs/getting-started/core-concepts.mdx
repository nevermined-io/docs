...
<CardGroup cols={2}>
  <Card title="Payment Method" icon="credit-card">
    Choose whether customers pay in [**Fiat**](/docs/integrate/patterns/fiat-payments) (USD or EUR via Stripe) or [**Crypto**](/docs/integrate/patterns/stablecoin-payments) (stablecoins like USDC or EURC).
  </Card>
  ...
  <Card title="Settlement" icon="coins">
    ...
    - **Fiat**: via Stripe or card delegation ([NVM Pay](/docs/products/nvm-pay/overview))
...
<AccordionGroup>
  ...
  <Accordion title="Pay-as-you-go">
    Charge the subscriber's payment method on each request, no upfront purchase:

    - **Per-request settlement**: charge as each request happens
    - **Card delegation**: subscribers enroll a card via [NVM Pay](/docs/products/nvm-pay/overview) and agents charge it directly
...
<Steps>
  ...
  <Step title="Customers purchase access">
    Subscribers buy a plan (fiat or crypto) or enroll a card for pay-as-you-go via [NVM Pay](/docs/products/nvm-pay/overview).
  </Step>
...
<CardGroup cols={3}>
  <Card title="Fiat Payments" icon="credit-card" href="/docs/integrate/patterns/fiat-payments">
    Accept credit cards via Stripe checkout. No crypto wallet needed, the fastest path for mainstream and enterprise adoption.
  </Card>
  ...
  <Card title="Card Delegation (NVM Pay)" icon="id-card" href="/docs/products/nvm-pay/overview">
    Subscribers enroll a Visa or Stripe card and create spending mandates. Agents charge directly against these mandates via x402, with per-transaction ceilings, usage limits, and expiration controls.
  </Card>
</CardGroup>
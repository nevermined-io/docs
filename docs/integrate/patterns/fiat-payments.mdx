...
Accept credit cards in **USD or EUR**, with no crypto wallet required. Fiat payments are the fastest path to mainstream adoption. Your users can start paying with a card they already have, either through Stripe Checkout (one-time plan purchase) or by enrolling their card via [card delegation](/docs/products/payments/overview) for autonomous agent charging.
...
- **Agent-to-Agent**: An agent owner enrolls a credit card via [card delegation](/docs/products/payments/overview) (Visa, Stripe, or Braintree) and creates a spending delegation. The agent can then make autonomous payments on behalf of the owner, charging the card per request within the defined limits.
...
<Note>
Card delegation is what enables fiat-based agent-to-agent payments. An agent owner enrolls their card and sets spending limits, and the agent can autonomously pay for services from other agents, all settled in fiat. See the [Payments FAQ](/docs/products/payments/faq) for common questions.
</Note>
...
<CardGroup cols={2}>
  <Card title="Agent Cards" icon="credit-card" href="/docs/products/payments/overview">

...
When a subscriber's credit balance runs low, the facilitator can automatically purchase more credits on their behalf at settlement — no manual balance checks required. This keeps agent-to-agent workflows running without interruption, bounded by the delegation's `spendingLimitCents`.

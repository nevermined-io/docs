...
router.post("/query", validatePayment, async (req: Request, res: Response) => {
...
describe("Payment Middleware", () => {
  it("returns 402 without payment proof", async () => {
    const res = await request(app).post("/api/query").send({ prompt: "test" })
...
<Accordion title="Error Handling" icon="triangle-exclamation">
  - [ ] Don't expose internal errors to clients - [ ] Log all payment validation
  errors - [ ] Implement retry logic for transient failures - [ ] Set up
  alerting for payment failures
</Accordion>

{" "}
<Accordion title="Performance" icon="gauge-high">
  - [ ] Cache Payments instance (singleton) - [ ] Consider caching validation
  results briefly - [ ] Monitor validation latency - [ ] Use connection pooling
</Accordion>

...
<Warning title="Sensitive data in traces">
  ...
  A too-short token (≤ 20 chars — almost always a misconfiguration, since real x402 access tokens are JWTs) is **redacted, not exported**: only `<first 4>…(short)` is surfaced and a warning is logged, so a wrong value passed as the token never lands in a trace.
...
If a span failure ever occurs during metadata building or attachment, observability is silently dropped — the payment flow itself is never interrupted. Settlement receipts persist (via `last_settlement()` in TypeScript / `last_settlement()` in Python) regardless of whether the span emit succeeded.

For the full module reference, see the [Python LangChain module reference](/docs/api-reference/python/langchain-module#observability-with-langsmith). The TypeScript helper surface is exported from `@nevermined-io/payments/langsmith`.

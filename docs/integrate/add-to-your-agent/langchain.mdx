...
Working examples with server, client, and decorator demos:

<Tabs>
  <Tab title="TypeScript">
    - [langchain-agent-ts](https://github.com/nevermined-io/hackathons/tree/main/agents/langchain-agent-ts) — LangChain + `bindTools()` tool-call loop
    - [langgraph-agent-ts](https://github.com/nevermined-io/hackathons/tree/main/agents/langgraph-agent-ts) — LangGraph `createReactAgent`

    Each includes:
    - `src/demo.ts` — Wrapper-based demo (direct tool invocation)
    - `src/server.ts` — Express server with `paymentMiddleware`
    - `src/client.ts` — HTTP client with full x402 payment flow
  </Tab>
  <Tab title="Python">
    - [langchain-agent](https://github.com/nevermined-io/hackathons/tree/main/agents/langchain-agent) — LangChain + `bind_tools()` tool-call loop
    - [langgraph-agent](https://github.com/nevermined-io/hackathons/tree/main/agents/langgraph-agent) — LangGraph `create_react_agent`

    Each includes:
    - `src/demo.py` — Decorator-based demo (direct tool invocation)
    - `src/server.py` — FastAPI server with `PaymentMiddleware`
    - `src/client.py` — HTTP client with full x402 payment flow
  </Tab>
</Tabs>

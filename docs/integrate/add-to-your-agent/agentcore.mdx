...
└────┬─────┘                              │  @requires_payment│
     │                                    └────────┬─────────┘
     │                                             │
     │  1. Call agent — no payment-signature        │
     │────────────────────────────────────────────>│
     │                                             │
     │     402 PaymentRequired                     │
     ...
     │<────────────────────────────────────────────│
     │                                             │
     │  2. Client decodes PaymentRequired,         │
     │     extracts planId from accepts[]          │
     │                                             │
     │  3. Client acquires x402 token via SDK      │
     │     payments.x402.get_x402_access_token()   │
     │                                             │
     │  4. Call agent with payment-signature header │
     │────────────────────────────────────────────>│
     │                                             │
     │     - Verify permissions (pre-flight)       │
     │     - Execute handler (agent work)          │
     │     - Settle (burn credits)                 │
     │                                             │
     │  5. Response + payment-response header      │
     │     (settlement receipt, base64 JSON)       │
     │<────────────────────────────────────────────│
     │                                             │
```
...
payments = Payments.get_instance(
    PaymentOptions(
        ...
        environment=os.environ.get("NVM_ENVIRONMENT", "testing"),
    )
)
...
```
Decorator config: credits=1
                    │
                    ├── VERIFY: "does subscriber have at least 1 credit?"
                    │
                    ├── EXECUTE: handler runs, returns response
                    │
                    └── SETTLE: burn credits
                         │
                         ├── If response has _meta.creditsToCharge: 5 → burn 5
                         └── If response has no override → burn 1 (config default)
```
...
```bash filename=".env"
# Nevermined (required)
NVM_API_KEY=nvm:your-publisher-api-key

...
<Tabs>
  ...
  // Generate the x402 access token. `delegationConfig` is required for
  // both crypto (nvm:erc4337) and fiat (nvm:card-delegation) plans —
  // omitting it surfaces BCK.X402.0005 at runtime.
  ...
  ```python
    ...
    # Generate the x402 access token (delegation_config is required)
...
<Tip>
Want subscribers to pay with a regular credit card instead of stablecoins? [NVM Pay](/docs/products/nvm-pay/overview) lets subscribers enroll a Visa, Stripe, or Braintree card and create spending delegations that agents charge directly.
</Tip>

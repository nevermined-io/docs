...
> In the code below we use “agent” terminology because it maps to the underlying object model, but you can think of it as _the service/resource you’re charging for_.
...
<Tabs>
  <Tab title="TypeScript">```bash npx ts-node register-agent.ts ```</Tab>
  <Tab title="Python">```bash python register_agent.py ```</Tab>
</Tabs>
...
<Card
  ...
  icon="python"
  href="/docs/integrate/add-to-your-agent/fastapi"
>

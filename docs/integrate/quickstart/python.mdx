...
<Tip>
For agents that run continuously, set up a delegation when generating the access token — the facilitator tops up credits automatically when the balance runs out, with no manual `order_plan()` loop needed. See [Automatic Credit Top-Ups](/docs/integrate/patterns/top-up).
</Tip>

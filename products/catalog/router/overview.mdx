...
<Note>
The rate is returned as `fee.bps` on every payment response — `200` basis points over a 10,000 denominator. Read it from there rather than hard-coding it.
</Note>

## Environment

To pay Catalog services, use **Live** — every listed service settles on mainnet, and the sandbox Router funds testnets only, so no listed service is payable there.

| Environment | API base | Purpose |
| --- | --- | --- |
| **Live** | `https://api.live.nevermined.app` | Real payments to Catalog services |

<Warning>
Both rails settle **real value on public networks**. The Router is a payment system, not a simulator — a live call moves real funds to a real merchant. Rehearse with the cheapest sub-cent service and keep your first Delegation cap small.
</Warning>
...
<Steps>
  ...
  <Step title="A funded wallet">
    Both rails **pull** from your own wallet, so it must already hold the payment asset on the network you're paying on. The Router never holds or moves float on your behalf.
  </Step>
</Steps>

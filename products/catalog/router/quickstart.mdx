...
<Note>
Driving this from an agent harness? [Using the Catalog from a harness](/products/catalog/from-a-harness) is the code-free companion to this page — the same flow, plus the safety habits (idempotency across retries, soft stops below your cap, funding for merchant + fee) an autonomous buyer should follow.
</Note>

Set your environment once. The Catalog is **Live-only for payment** — every listed service settles on mainnet, and the sandbox Router funds testnets only (apart from two free testnet test merchants kept there for rehearsal, see [the Commerce MCP page](/products/catalog/mcp#1-connect-your-host)) — so point at the live host:
...
Both rails **pull**: the merchant takes the money from your own wallet. A Delegation authorizes the spend, but the wallet has to actually hold the funds. Find its address on the Delegation:
...
| MPP | Tempo Moderato (`42431`) | PathUSD — `0x20c0000000000000000000000000000000000000`, self-funded via [one RPC call](/products/catalog/router/rails-mpp#funding-your-wallet) |

The address is the same on every EVM chain — it's one key-derived wallet, so there is nothing extra to create on Tempo. But **the balances are per-chain**: funding Base does nothing for an MPP payment. See [the x402 rail](/products/catalog/router/rails-x402) and [the MPP rail](/products/catalog/router/rails-mpp) for the full picture.

<Note>
**Funding is manual today.** There is no on-ramp — you cannot top this wallet up from a card or from a Nevermined credit balance, and the agent cannot refill itself. When the wallet runs dry, payments stop until a human sends more: on MPP with a clean `402 BCK.ROUTER.0009`, on x402 [less kindly](/products/catalog/router/rails-x402#funding-your-wallet), because that rail doesn't check the balance before signing. Budget for that when you decide how much to fund — and fund for the merchant's price **plus** any [routing fee](/products/catalog/router/how-it-works#the-routing-fee). An agent is autonomous *after* it is funded, and only until the balance runs out.
</Note>

<Warning>
Always read this address back from the live Delegation rather than reusing one you saved. A stale address is the single most common cause of a confusing `402 BCK.ROUTER.0009`. On MPP-tempo the error does name the wallet and the chain it checked — so compare it against the one you funded — but it never echoes the **balance**, and on x402 there is no pre-check and no `0009` at all. [`wallet_balance`](/products/catalog/mcp#6-check-your-spend-and-your-balance) in the Catalog MCP reads the amount back.
</Warning>
  ...
  <Tab title="curl">
    ...
    curl -sX POST "$NVM_API_URL/api/v1/router/route" \
      ...
      -d '{
        ...
        "method": "GET",
        "requestId": "fetch-example-resource-v1"
      }'
    ```
  </Tab>
...
That applies to the snippet above, which uses a fixed id: **run it a second time and you get `409 BCK.ROUTER.0002`**, carrying the original `paymentId`, rather than the response shown. That is the guarantee working, not a failure — change the id to buy again.
</Note>
...
Every payment across every protocol is on one ledger — see [the ledger page](/products/catalog/router/ledger) for filters, CSV export, and the aggregate summary.
...
[Guardrails](/products/catalog/router/guardrails) covers every code and, more usefully, why widening a cap in response to a refusal is almost always the wrong move.

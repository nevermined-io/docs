...
### Per-endpoint request size

Some services accept smaller request bodies than the Router does. The catalog records that limit as `maxRequestBytes` (UTF-8 bytes) on each endpoint of the service detail (`GET /api/v1/catalog/services/{slug}`) and in the MCP `get_service` result. Every Locus gateway (`*.mpp.paywithlocus.com`) publishes `8000`: its load balancer refuses a larger body before it ever asks for payment.

When you call a service by `slug`, the Router checks your body against that limit **before contacting the service**. A larger body returns **`413 BCK.ROUTER.0027`** with `bodyBytes` and `maxRequestBytes` in `params`. Nothing was paid, so shrink the body or pick another service. `POST /router/select` (and the MCP `route_by_intent`) skips endpoints whose limit is below the `body` you send, so a long prompt is routed to a service that takes it. An endpoint without `maxRequestBytes` has no known limit, which is not the same as having none.
...
Across the whole Router surface, only `0006`, `0007`, `0020` and quote-only `0028` are worth retrying automatically. The rest are decisions, and retrying them unchanged will produce the same answer. On the *paying* path the set is narrower still — see below.

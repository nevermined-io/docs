...
On **x402 there is no balance check anywhere.** The Router signs and reserves your budget, and the shortfall surfaces only when the transfer is attempted on-chain — the payment lands as `Failed` and the reserved **merchant** leg is **not** given back. Only a routing fee reserved on top of it can be credited back, and only on [the closed list of paths that release one](/products/catalog/router/how-it-works#when-a-fee-is-not-collected). Fund for the merchant's amount **plus** any [routing fee](/products/catalog/router/how-it-works#fund-for-the-amount-plus-the-fee), which is a second transfer from the same wallet.

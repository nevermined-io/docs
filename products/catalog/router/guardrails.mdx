...
description: "What the Router enforces on a payment, what it refuses outright, and every error code it can return."
icon: "shield-check"
---
...
| | |
| --- | --- |
| **Cap** | The spend, converted to cents and rounded up, must fit in the remaining budget — including any [routing fee](/products/catalog/router/how-it-works#the-routing-fee), which is reserved together with the merchant amount |
| **Per-call ceiling** | Optional `maxTotalCents` on Router JSON calls: a non-negative safe integer. It limits one fee-inclusive, whole-cent cap reserve (merchant price plus any collectable Router fee, rounded once); the Delegation cap remains the overall limit |
...
Delegation cap, expiry, or status failures give you **`402 BCK.ROUTER.0003`**. Check the current state any time with `GET /api/v1/delegation/{id}` — `remainingBudgetCents` and `expiresAt` tell you which it was. A quote above `maxTotalCents` gives **`402 BCK.ROUTER.0018`** without a charge or budget reserve, although signing may already have occurred; raise that per-call ceiling only if the call is intended.

### Check the price first: `POST /router/quote`

`maxTotalCents` refuses an over-price call, but it runs after the rail has prepared the payment, so it is not free price discovery. To learn the price first, **quote** the call with the same body as `/route` (no `requestId`; `delegationId` optional):

```bash
curl -sX POST "$NVM_API_URL/api/v1/router/quote" \
  -H "Authorization: Bearer $NVM_API_KEY" -H "Content-Type: application/json" \
  -d '{ "slug": "webscrape-pro", "path": "scrape", "method": "POST", "body": { "url": "https://example.com" } }'
```

The Router makes the same unpaid probe `/route` makes, selects the option a payment would (MPP first) and prices it with the routing fee — then stops. **Nothing is signed, minted, recorded or reserved.** The `200` answer carries `paymentRequired`, `optionSet` (`delegation` when you sent a `delegationId`, else `deployment` — what a personal crypto Delegation selects), `protocol`, and the same `settlement` and `fee` objects `/route` returns. Route with `maxTotalCents` set to `fee.capChargedCents` so a price that moved in between is refused rather than paid. A quote checks neither the remaining cap nor the wallet balance; a failed chain read it depends on is `503 BCK.ROUTER.0028`, retryable.
...
A cap authorizes a spend; it doesn't provide funds. Both rails **pull** from your own wallet, so it has to hold the asset on the target network before you call — and what happens when it doesn't differs by rail.
...
On **x402 there is no balance check anywhere.** The Router signs and reserves your budget, and the shortfall surfaces only when the transfer is attempted on-chain — the payment lands as `Failed`, and the reserved **merchant** leg is released once the reconciler has confirmed the authorization expired **unconsumed**, not at the moment of failure. Your cap stays reserved until then. Only a routing fee reserved on top of it can be credited back, and only on [the closed list of paths that release one](/products/catalog/router/how-it-works#when-a-fee-is-not-collected). Fund for the merchant's amount **plus** any [routing fee](/products/catalog/router/how-it-works#fund-for-the-amount-plus-the-fee), which is a second transfer from the same wallet.
...
## What the merchant sees of you

`/proxy` relays a live request, so the headers reaching it are not only the ones you set — every proxy between you and the Router appends its own (`X-Forwarded-Host`, `X-Real-Client-IP`, `traceparent`, …). Those describe **our** infrastructure, not your request, and a merchant has no business receiving them. So `/proxy` forwards an allowlist of standard request headers and drops everything else, `Cookie`, `Origin` and `Referer` included. Add what a merchant genuinely needs by stating its value — `X-Router-Forward-x-api-key: sk-…` sends `x-api-key: sk-…` upstream. That channel carries a value rather than naming a header to replay, so it can never be used to read back something our own infrastructure set: nothing replayed off the wire is forwarded except the allowlist.

The Router also adds nothing of its own on the way out: no `Forwarded` or `X-Forwarded-For` is synthesised, so your IP never reaches the merchant.

`POST /route` carries no such risk and applies no allowlist — its `headers` are a JSON object you wrote, so it forwards exactly what you ask for.
...
Every code below also appears in the canonical [API error catalogue](/development-guide/api-errors/codes#bck-router), which carries the `category` and `retryable` fields your client can branch on without parsing a message. [Error handling](/development-guide/api-errors/overview) describes the envelope they arrive in.
...
| `BCK.ROUTER.0008` | 403 | Legacy API key. Create a new one — see [the quickstart](/products/catalog/router/quickstart#1-get-an-api-key). | No |
...
Across the whole Router surface, only `0006`, `0007` and `0020` are worth retrying automatically. The rest are decisions, and retrying them unchanged will produce the same answer. On the *paying* path the set is narrower still — see below.

<Warning>
**`0010` is a 500 that must never be retried** — which makes it the exception to "retry the 5xx". In fact `0006`, the one retryable 500, is raised only by the payments *summary* read and never by a payment, so on the paying path **`0007` and `0020` are the only codes worth retrying** (with backoff).
...
4. **Check the price before you commit.** `settlement.approxCents` on the response tells you what the merchant charged, and `fee.capChargedCents` what your cap was actually debited — the two differ when a [routing fee](/products/catalog/router/how-it-works#the-routing-fee) applies. A run of sub-cent calls still burns a cent of budget each.
...
<CardGroup cols={2}>
  ...
  <Card title="Full error catalogue" icon="circle-exclamation" href="/development-guide/api-errors/codes#bck-router">
    The catalogue entry for each `BCK.ROUTER.*` code, with its category and remediation hint.
  </Card>
  <Card title="Router API reference" icon="code" href="/api-reference/router/mint-payment-credential">

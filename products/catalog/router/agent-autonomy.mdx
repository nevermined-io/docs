...
The catalog is public. No key, no account, no signup — one JSON feed of every listed service, filtered locally:

```bash
curl -s https://nevermined.app/catalog/ai-catalog.json \
  | jq -r '.services[] | select((.title + " " + .description) | ascii_downcase | contains("crypto"))
           | "\(.title)  [\(.protocol)]  \(.priceLabel)"'
```

```
Alpha Vantage  [mpp]  $0.008
CoinGecko via MPP  [mpp]  $0.06
Otto AI x402  [x402]  From $0.001/call (USDC)
AgentOracle  [x402]  $0.01
…
```

Every service lists its protocol, network and price up front. Each entry carries everything needed to place a call — the `slug`, each endpoint's `path` and `method`, and a `priceLabel`:

```bash
curl -s https://nevermined.app/catalog/ai-catalog.json \
  | jq '.services[] | select(.slug == "coingecko-via-mpp")
        | {slug, title, protocol, network, priceLabel, endpoints: [.endpoints[0] | {path: (.invokePath // .path), method, priceLabel}]}'
```
...
<Note>
The feed is the whole catalog in one document — there are no query parameters, so filter it on your side. `/api/v1/catalog/services` isn't a public integration and returns `403`; for server-side search, use the Catalog MCP's `search_services` (see [Discovering AI Services](/products/catalog/discover#search-server-side-with-the-catalog-mcp)).
</Note>
...
- **The agent holds nothing.** No wallet, no private key, no merchant credentials. Funds sit in a wallet it cannot reach except through a capped Delegation.
...
- **Read the wallet address back from the live Delegation** (`providerPaymentMethodId`), never from anything you wrote down — funding a stale address fails as `402 BCK.ROUTER.0009`. On MPP-tempo that error names the wallet and chain it checked, so compare the two; it never echoes the balance, and x402 raises no `0009` at all.
...
- **Check the catalog is loaded**, not serving placeholder rows — compare the feed's `total` and look at the `endpoints`.

...
- **Read the wallet address back from the live Delegation** (`providerPaymentMethodId`), never from anything you wrote down — funding a stale address fails as `402 BCK.ROUTER.0009`. On MPP that error names the wallet and chain it checked, so compare the two; it never echoes the balance, and x402 raises no `0009` at all.

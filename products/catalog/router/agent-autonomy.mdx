...
<Note>
`?search=` matches words in titles, descriptions and tags — **not** slugs. To fetch one known service, use the by-slug route above. `limit` is capped server-side at 20, so paginating the whole catalog client-side isn't the answer either.
</Note>

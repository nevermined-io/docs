...
| `X-Router-Forward-Headers` | Optional — comma-separated extra request headers to forward. |
...
Need another one — a merchant keyed on `X-Api-Key`, say? Name it: `X-Router-Forward-Headers: x-api-key, x-tenant-id`. Naming a header is a request, not an override: hop-by-hop headers, the `X-Router-*` control headers, your `Authorization` and `Cookie` stay un-forwardable however they are named.

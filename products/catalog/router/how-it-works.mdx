...
The payment is made and the call keeps running on the Router, up to its usual 120-second limit. The routing fee follows the same rule as a call you waited for: collected when the service answers `2xx`, released otherwise. Read the result with the same API key:

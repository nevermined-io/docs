...
Those fields are on **every** row of that endpoint, not just Router ones, and they read `"0"` on a plan purchase or a crypto settlement — which pay no routing fee *on top of* their price. Do not carry the formula above onto those rows: a plan purchase can carry a platform margin taken *out of* the price, which `feeCents` deliberately does not describe. That figure is `nvmMargin`, published per-plan on `POST /api/v1/transactions/search`.

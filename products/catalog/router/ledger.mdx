...
The routing fee is **2%** (`feeBps: 200`). **Mode A now applies it to every call**; mode B applies it too, but only settles once the merchant returns 2xx — otherwise it is [`Released`](#fee-lifecycle), *unless* its outcome could not be adjudicated, in which case it stays `Submitted` and charged. Three cases legitimately read `feeStatus: "None"` with a zero fee: an **MPP** payment, since that rail cannot carry a fee at all; a deployment with **no rate configured**, which is the shipped default and what the sample record above shows; and a payment whose fee rounds below one atomic unit (the case in the warning above).

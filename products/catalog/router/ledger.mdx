...
The routing fee is **5%** (`feeBps: 500`), and both modes collect it on every call. Two cases legitimately read `feeStatus: "None"` with a zero fee: an **MPP** payment, since that rail cannot carry a fee at all, and a payment whose fee rounds below one atomic unit (the case in the warning above).

...
| `feeFailureReason` | Why the fee didn't collect — or, on a `None` row, why one was never quoted. Diagnostic text for a human, **not a stable contract**: branch on `feeStatus`, never on this string. `null` when no rate is configured and while a quoted fee is still on its happy path. See below. |
...
When a rate is configured, the routing fee is **2%** (`feeBps: 200`), and it's collected on **both** the x402 and MPP rails — by [two different mechanisms](#auditing-the-fee-leg) their keys reflect. **Mode A now applies it to every call**; mode B applies it too, but only settles once the merchant returns 2xx — otherwise it is [`Released`](#fee-lifecycle), *unless* its outcome could not be adjudicated, in which case it stays `Submitted` and charged. Three cases legitimately read `feeStatus: "None"` with a zero fee: a **card**-rail payment, since that rail can't carry a fee leg; a deployment with **no rate configured**, which is the shipped default and what the sample record above shows; and a payment whose fee rounds below one atomic unit (the case in the warning above).

**`feeFailureReason` says which of those you are looking at.** A `None` row produced while a rate *was* configured carries a `fee-not-quoted: …` string naming the reason — no destination for the settlement chain, no usable fee submitter, a rail that cannot carry a fee leg, or an amount too small to transfer. A `None` row with `feeFailureReason: null` is the shipped default: no rate is configured, so no fee was ever asked for. The two used to be indistinguishable on the wire.

**Use a stable `requestId`, and a retry costs you nothing extra.** Because mode A now bills every call, the way to avoid paying twice for one purchase is to make the purchase idempotent: reuse **one** `requestId` across every retry of it. A retry then returns `409 BCK.ROUTER.0002` with the original `paymentId` instead of minting a second payment, so there is only ever one fee.

Derive that id from the work you are doing, not from a fresh `uuid4()` per HTTP attempt — a new id on each attempt is indistinguishable from a new purchase, and will be minted and billed as one.

<Note>
**This is why merchant amounts and spent budget don't reconcile on their own.** `amount` on this record is the merchant leg, so the combined cap charge isn't derivable from the ledger alone. It's recorded as `amountCents` on `GET /api/v1/delegation/{id}/transactions`, and that same row breaks it out as `feeCents`, `feeAtomic` and `feeBps` — so **on a Router row**, `amountCents - feeCents` is the merchant leg in cents.

Those fields are on **every** row of that endpoint, not just Router ones, and they read zero on a plan purchase or a crypto settlement — which pay no routing fee *on top of* their price. Do not carry the formula above onto those rows: a plan purchase can carry a platform margin taken *out of* the price, which `feeCents` deliberately does not describe. That figure is `nvmMargin`, published on `POST /api/v1/transactions/search` — one value per transaction, not per plan.

If the fee is later [`Released`](#fee-lifecycle), that row's `amountCents` is reduced by the fee and `feeReleased: true` is set — but the `feeCents` breakout **stays**. So on a released row `amountCents` is *already net of the fee*: don't subtract it a second time, and don't add it back to reconstruct a total that has moved. It does **not** follow that what remains is the merchant leg — a payment whose merchant reserve was also released has that deducted from the same field, and reads `status: "failed"`.

One exception to the flag, and it is the [sub-cent case](#the-routing-fee-on-a-record) above: `feeReleased: true` is set only when a cent was actually reserved. A fee too small to reserve one is released without ever setting it, and leaves `amountCents` untouched — so such a row reads `false` here while `GET /router/payments` reports the fee `Released`. The arithmetic still holds; the two labels just disagree.
</Note>

### Auditing the fee leg

`feeTxHash` and `feeNonce` are what make the fee movement auditable on its own, and each answers a different question — and which one anchors a given fee follows the rail: an **MPP** fee is a native transfer, keyed by `feeTxHash`; an **x402** fee is an EIP-3009 authorization, keyed by `feeNonce`.
...
| `feeStatus` | Meaning |
| --- | --- |
| `None` | No fee was charged — neither an atomic fee nor a cent of cap. The normal state while no rate is configured, and also what you get when a rate *is* configured but the fee provably can't be collected on this path. Either way nothing was reserved, so there's nothing to give back later. `feeFailureReason` tells the two apart. |
...
The CSV carries the same fields as the JSON, and new columns are only ever **appended** to the header row — so a consumer parsing by column index is unaffected by their arrival. `feeFailureReason` is currently the last column.

<Note>
**`feeFailureReason` is normalised before it is published.** Whitespace is collapsed to single spaces — so one payment is always one CSV record, even though some of the underlying errors are multi-line — and any embedded `scheme://…` URL is replaced with `[url]`, because part of this text comes from third parties we do not control. Read it as a human-facing diagnostic, and key your logic on `feeStatus`.
</Note>

...
<Note>
The Catalog lists services you **buy from**. If instead you want your own API to *get paid* by callers, that's the seller side — see the **[x402 Facilitator](/products/x402-facilitator/overview)**. You can be on both sides at once, and you can even **[publish your own agent](/products/catalog/discover#publish-your-agent)** into the Catalog so other buyers discover it.
</Note>
...
One agent, one budget, one API key — reaching many independent services it never had to sign up for:

```mermaid
flowchart LR
    Agent["Your agent"]
    Catalog["Nevermined Catalog<br/>discover services"]
    Router["Nevermined Router<br/>one budget, one API key"]
    S1["AI Service A · x402"]
    S2["AI Service B · MPP"]
    S3["AI Service C · x402"]

    Agent -->|"1 · browse and pick"| Catalog
    Agent -->|"2 · route the payment"| Router
    Router --> S1
    Router --> S2
    Router --> S3
```

The services on the right are unrelated to each other and to Nevermined — different providers, different payment rails, no shared account. Your agent treats them uniformly because the Router, not the agent, speaks each rail.

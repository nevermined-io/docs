...
The Catalog is **Live-only**. Every listed service settles on **mainnet** (Base and Tempo), so all discovery and payment go through the live host:
...
<Warning>
Both rails settle **real value on public networks** — a Catalog call moves real funds to a real merchant. There is no sandbox Catalog: the sandbox Router funds testnets only, so no listed service is payable there. Rehearse with the cheapest sub-cent service and keep your first Delegation cap small.
</Warning>

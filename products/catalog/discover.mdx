...
Base URL is the live environment host: `https://api.live.nevermined.app`. Discovery works against either host — they serve the same catalog — but the Catalog is **Live-only for payment**: listed services settle on mainnet and a sandbox deployment funds testnets only, so paying one from sandbox fails with `400 BCK.ROUTER.0001` (see [an agent buying on its own](/products/catalog/router/agent-autonomy)).

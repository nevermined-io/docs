...
Base URL is the live environment host: `https://api.live.nevermined.app`. Discovery works against either host — they serve the same catalog — but the Catalog is **Live-only for payment**: listed services settle on mainnet and a sandbox deployment funds testnets only, so paying one from sandbox fails with `400 BCK.ROUTER.0001` (see [an agent buying on its own](/products/catalog/router/agent-autonomy)). The one exception is the pair of free testnet test merchants the sandbox Catalog carries for rehearsal — see [the Commerce MCP page](/products/catalog/mcp#1-connect-your-host).

### List services

```bash
curl -s "https://api.live.nevermined.app/api/v1/catalog/services?category=Search%20%26%20Research&protocol=x402"
```

| Param | Notes |
| --- | --- |
| `search` | Words in any order, matched across title, descriptions and tags, most relevant first. If no listing contains every word, listings containing any of them are returned |
...
```bash
curl -s "https://api.live.nevermined.app/api/v1/catalog/categories"
```
...
```bash
curl -s "https://api.live.nevermined.app/api/v1/catalog/services/<slug>"
```

...
Read `get_service` first, then call **`pay_service`** with the `payServiceArgs` of the endpoint you want — the `slug`, its `path`, and the `body` and `headers` the vendor needs:

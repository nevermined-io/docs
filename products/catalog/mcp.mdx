...
<Steps>
  <Step title="Add the server URL">
    That is the entire configuration. No client ID, no client secret, no API key — your assistant discovers everything else from the server.
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="The agent says the budget is spent, or the grant expired">
    That is the Delegation, not the connection — see [When the grant runs out](#when-the-grant-runs-out). A Delegation is never widened or extended in place: re-run the connect step and approve a new cap.
  </Accordion>

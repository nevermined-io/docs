...
| `get_payment_result` | paid | Read a paid call's result by `paymentId` — for a `pay_service` that answered `pending`, or a response you lost. Charges nothing; results are kept for 24 hours. |
...
<Frame caption="Claude, ChatGPT and a LangSmith Fleet agent each buy real services from a $5 budget approved once — recorded on live, with real money.">
  <iframe
    src="https://www.youtube.com/embed/KJxlW6qI6So"
...
[Claude](https://youtu.be/uW-_V-dOeSw) · [ChatGPT](https://youtu.be/fI_llp5BBYM) · [LangSmith Fleet](https://youtu.be/A7HcBsnRYG8).
...
| `{ "status": "pending", "paymentId": "…", "nextTool": "get_payment_result" }` | Not a refusal: the payment was made and the service is still working (it took longer than 45 seconds). Call **`get_payment_result`** with the `paymentId` until it reports `Ready` or `Failed`. Don't call `pay_service` again for it — the same call returns the same `pending`, and a new `requestId` would buy it twice. |

...
Discovery is **free** — no credential needed. Picking or paying for a service, reading your ledger and reading your wallet are **paid** tools: they need a credential, either your API key or an OAuth grant your host obtains for you.
...
- **`get_service`** with `{ "slug": "<a-slug-from-the-search>" }` — one service's protocol, price label, and its **`requestShape`**: every callable endpoint with its `path`, HTTP `method` and price, each carrying the exact `payServiceArgs` to hand to `pay_service`. Many endpoints also carry a **`requestSchema`**: the JSON Schema of the request *as the merchant declares it* (`properties.query` is what goes in `search`, `properties.body` what goes in `body`), sanitised by Nevermined but not verified, with its `requestSchemaSource`. `noParameters: true` marks an endpoint that takes none. A `requestExample` is graded by **`exampleEvidence`**: `paid-run` (a bounded paid call returned the recorded `responseFields`), `challenge` (the request drew a payment challenge from a gateway that validates requests before pricing them), or `docs` (from provider documentation, unverified). Only a `paid-run` or `challenge` example is pre-filled into `payServiceArgs`; a `docs` example is a reference to adopt deliberately. Where the Catalog has nothing for an endpoint, the `requestShape` note says so rather than leaving you to guess, and a service the Catalog knows nothing about gets an explicit `unknown — the catalog holds no request schema` note.

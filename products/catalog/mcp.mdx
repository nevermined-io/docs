...
- **`get_service`** with `{ "slug": "<a-slug-from-the-search>" }` — one service's protocol, price label, and its **`requestShape`**: every callable endpoint with its `path`, HTTP `method` and price, each carrying the exact `payServiceArgs` to hand to `pay_service`. An endpoint with a `quote` has a live price: the merchant's price for one call, on the rail and network the Router pays it on. The Router's routing fee is charged on top; for the total, or the price under your own Delegation, use `POST /api/v1/router/quote`. The Catalog stores no request-body schema, and `requestShape` says so (`bodySchema: null`) rather than leaving you to guess; a service the Catalog knows nothing about gets an explicit `unknown — the catalog holds no request schema` note.

...
- **`wallet_balance`** — what your funding wallet actually *holds*. Different question, different answer: the ledger says what you've spent, and your Delegation says what you're *allowed* to spend, but [both rails pull from your wallet](/products/catalog/router/how-it-works) and neither of those tells you whether the money is there.
...
```json
{
  "address": "0x1234…abcd",
  "networks": [
    { "network": 8453, "balances": [{ "symbol": "USDC", "decimals": 6, "atomic": "1500000", "formatted": "1.5" }] },
    { "network": 4217, "balances": [{ "symbol": "USDC", "decimals": 6, "atomic": "0", "formatted": "0" }] }
  ]
}
```
...
`pay_service` returning `{ "error": "payment_failed", "code": "BCK.ROUTER.0009" }` means the wallet couldn't cover the charge — and that error [deliberately names neither the wallet nor the balance it checked](/products/catalog/router/quickstart#3-fund-your-wallet), so it can't tell you *which* network is short. `wallet_balance` is how you find out without leaving the chat.

What it can't do is fix it. There's no on-ramp and your agent can't top its own wallet up, so a genuine shortfall is a stop condition: a human funds the wallet on the right chain, and the agent retries. Widening your Delegation cap does nothing here — a cap authorizes a spend, it doesn't supply the funds.
...
| `{ "error": "payment_failed", "code": "BCK.ROUTER.0009", … }` | Your wallet couldn't fund the payment. Nothing was signed. Call **`wallet_balance`** to see which network is short — the error doesn't say. A human has to fund it; there's no on-ramp and no cap to widen. |

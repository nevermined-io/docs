...
Nine tools. Discovery is **free** — no credential needed. Picking or paying for a service, reading your ledger and reading your wallet are **paid** tools: they need a credential, either your API key or an OAuth grant your host obtains for you.
...
`route_by_intent` turns a plain-language need into the **single best payable service** in the Catalog, plus the ranked shortlist it was chosen from — so your agent doesn't search, compare listings and pick one itself. The ranking is deterministic: Nevermined's own relevance and quality signals, behind a payability gate (listed, healthy, not held by moderation, on x402 or MPP). No model reads the merchant listings, and when nothing passes the gate you get an explicit refusal, never a weak match.
...
| `requestId` | with `autoPay` | Idempotency key for the payment. Reuse it verbatim on any retry of the same purchase. |
| `delegationId` | with `autoPay`, on an API key | The Delegation to charge. An OAuth commerce grant brings its own Delegation, so leave it out there. |

With `autoPay: true` the call also takes `pay_service`'s request arguments — `path`, `method`, `search`, `body`, `headers`, `credentialHeader` and `maxTotalCents` — and applies them to whichever service wins. That is the catch: you're shaping a request for a service you haven't seen yet, and a bare call reaches the winner's base URL, which for a multi-endpoint API is a 404, not a purchase (see [step 5](#5-make-a-paid-call)). When the endpoint matters, propose first, read `get_service` on `chosen.slug`, then pay with `pay_service`.
...
- **`shortlist`** is the payable candidates in pick order: `relevance` (0–100) first, then `rankScore` — an opaque quality score, higher first, `null` until the service has been scored — to break ties. `healthy` is whether it passed the health check.
...
| `{ "status": "already_paid", "code": "BCK.ROUTER.0002", "paymentId": "…", … }` | A retry of a `requestId` that was already paid. **Not a new charge** — the original payment stands. | Treat the purchase as done; `paymentId` names it. Don't retry with a fresh `requestId`. |

Any other refusal on `autoPay` — the cap exceeded, a wallet short, a card declined — comes back as an error result carrying the Router's own `BCK.ROUTER.*` code, with the same meaning it has for `pay_service`: see [When a payment is refused](#when-a-payment-is-refused).

...
| `{ "status": "already_paid", "code": "BCK.ROUTER.0002", "paymentId": "…", … }` | A retry of a `requestId` that already has a payment on record. **Not a new charge** — and the vendor's response isn't replayed. | Find `paymentId` in `list_payments`. `Settled` means the purchase is done. `Failed` means nothing was delivered: buying again takes a **new** `requestId`, as a deliberate new purchase rather than a retry. |

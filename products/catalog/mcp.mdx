...
Nine tools. Discovery is **free** — no credential needed. Paying, reading your budget, your ledger and your wallet are **paid** tools: they need a credential, either your API key or an OAuth grant your host obtains for you. ("Paid" is about access, not price — `get_budget` and the ledger tools charge nothing.)
...
| `get_budget` | paid | Your spending budget: cap, spent, remaining, payments made and expiry. Charges nothing. Optional: `delegationId`. |
...
```json
{
  ...
  "budget": { "capCents": "500", "spentCents": "8.16", "remainingCents": "491.84", "currency": "usdc", "expiresAt": "2026-09-23T13:29:31.691Z", "transactionCount": 40, "maxTransactions": null, "status": "Active" },
  "source": "oauth_grant",
  "message": "Your spending budget is in place: delegation 121b880a… — cap 5.00 USDC, 0.0816 USDC spent, 4.9184 USDC remaining, valid until 2026-09-23T13:29:31.691Z. …",
...
- **`get_budget`** — what your Delegation has left: cap, spent, remaining, payments made (and the limit, if any) and expiry. It charges nothing, and it is the answer to *"how much is left?"* without leaving the chat.

```json
{
  "delegationId": "121b880a-…",
  "budget": { "capCents": "500", "spentCents": "1.632", "remainingCents": "498.368", "currency": "usdc", "expiresAt": "2026-10-01T00:00:00.000Z", "transactionCount": 8, "maxTransactions": 50, "status": "Active" },
  "message": "Spending budget for delegation 121b880a…: cap 5.00 USDC, 0.01632 USDC spent, 4.98368 USDC remaining, valid until 2026-10-01T00:00:00.000Z, 8 payments of 50 allowed."
}
```

Amounts are in **cents**. `capCents` is whole cents; `spentCents` and `remainingCents` can carry up to four decimals, because a Delegation is charged at 1/10,000 of a cent — a 0.204¢ call costs 0.204¢ of your cap, not a whole cent — and `spentCents + remainingCents` equals `capCents` while the budget is within its cap. The same `budget` block rides on `setup_delegation`'s answer and on **every paid `pay_service` result** (the running total *after* that call, with its `delegationId`), so the assistant can repeat a real figure instead of adding up its own calls. `budget: null` on a `pay_service` result means the budget could not be read that time, not that it is zero.

With no arguments `get_budget` reads the Delegation `pay_service` would spend from. That only includes budgets that can still pay, so once a cap is spent or expired it answers `{ "status": "no_active_delegation" }`; pass the `delegationId` from an earlier `pay_service` result to read that spent budget anyway.

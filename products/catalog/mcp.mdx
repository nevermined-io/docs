...
| --- | --- | --- |
| **MPP** | Your wallet was read *before* anything was signed and came up short. The error names the wallet and the chain — what it never says is **how much** is there. | `wallet_balance` supplies the amount. A human funds that wallet on that chain. |
| **MPP, card-funded** | Your card **issuer declined**. There is no wallet involved and nothing to top up. | `wallet_balance` doesn't apply. Use a different card. |
...
<Note>
`wallet_balance` reads your **personal** wallet. If your Delegation is backed by an [organization wallet](/products/catalog/router/overview), the Router pays from *that* wallet and this tool reports a different one.
</Note>
...
| `{ "error": "payment_failed", "code": "BCK.ROUTER.0009", … }` | The funding source couldn't cover the charge. Nothing was signed. **On the crypto rails** the wallet is short — the error names the wallet and chain but not the amount, so call **`wallet_balance`** for that; a human has to fund it, and there's no cap to widen. **On the card rail** the issuer declined: no wallet, nothing to top up, use another card. |

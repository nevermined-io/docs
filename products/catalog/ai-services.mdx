...
The full set also includes Sales & Business Intelligence, Social & Creator, Identity & Compliance, Infrastructure & Compute, Weather, and Travel. Browse or query the live set — the current categories and their counts come from the public feed, never a hard-coded list. See [Discovering AI Services](/products/catalog/discover).
...
The Catalog is also where you list a service **you** own so other buyers' agents discover and pay for it. From a **paid** organization (Starter or above) in the [Nevermined app](https://nevermined.app), publish one of your registered agents into the Catalog with a category, its callable endpoint, and presentation details (title, description, logo, tags, features).

For the step-by-step — the two ways to submit and what each one needs — see **[Publish to the Catalog](/products/catalog/publish-ai-service)**.
...
### Health is also judged on paid calls

The monitor's probe does not pay, so it proves your endpoint answers, not that a paid call delivers. The Catalog also reads the outcome of real purchases made through the [Router](/products/catalog/router/overview):

- **Hidden while payments don't go through.** If a listing's paid calls over the last 7 days all went undelivered, it is hidden from discovery. That takes at least 3 undelivered calls from at least 2 different buyers, and none settled. An undelivered call is one that failed, or one where your endpoint collected the payment and then answered with an error. One settled call brings the listing back. If there are no new calls, it comes back automatically after 3 days.
- **No uptime claim without a settled call.** If a listing has at least 3 paid calls from at least 2 buyers in the last 30 days and none of them settled, it stays listed but shows no "operational" status, uptime or last-healthy time until a call settles. Card payments made over MPP (Stripe) are not counted here, because that rail never records a settlement.

<Note>
Publishing requires an **active paid tier** (Starter or above) — see [Plans & Billing](/solutions/organizations/plans-and-billing). See [Agentic integration](/solutions/organizations/agentic-integration) for the organization API context, and [Register a plan and agent](/agents-guide/register-plan-and-agent) for creating the agent you'll list.
</Note>
...
<CardGroup cols={2}>
  <Card title="Discovering AI Services" icon="magnifying-glass" href="/products/catalog/discover">
    Browse the app, or read the public feed, to find what you need.
  </Card>

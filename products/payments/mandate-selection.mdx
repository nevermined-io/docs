...
<Steps>
  <Step title="Mode 1: Explicit Delegation ID">
    ...
    <Note>
      This is the **only** supported mode for Visa delegations. Auto-creation from the SDK requires `consumerPrompt` + `assuranceData`, which the SDK cannot produce — see the [Visa enrollment notes](/products/payments/card-enrollment).
    </Note>
  </Step>

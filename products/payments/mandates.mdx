...
| `amountCents` | Amount in cents. On a Router row this is the **combined** cap deduction — the merchant leg plus the routing fee below |
| `currency` | Currency code |
| `status` | `succeeded` or `failed`. Card-rail rows can carry the PSP's own status verbatim (e.g. Braintree's `SUBMITTED_FOR_SETTLEMENT`) |
...
| `feeCents` | Nevermined's routing fee in cents, already included in `amountCents`. `"0"` on a card or crypto row, which pay no routing fee on top of their price |
...
<Note>
The routing fee applies only to payments routed through the [Nevermined Router](/products/catalog/router/ledger#the-routing-fee-on-a-record); a plan purchase on the card rails reads `"0"` for all four. Nevermined's cut on such a purchase is carved *out* of the plan price rather than added on top, and is published separately as `nvmMargin` on `POST /api/v1/transactions/search`.
</Note>

...
The **Merchant Payment Protocol (MPP)** is an open HTTP-402 standard for agent-to-agent payments. If your endpoint is already protected by a Nevermined plan, you can advertise MPP as an accepted protocol on it — and every request is metered identically to x402: same plan, same credits, same delegation.

<Note>
This page is the **seller** side: your endpoint gets paid. For the opposite direction — paying an external MPP merchant through the Router — see [the MPP rail](/products/catalog/router/rails-mpp). Only the wire format is shared.
</Note>

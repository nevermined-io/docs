...
`amount` is the **merchant** leg only. If a [routing fee](/products/router/how-it-works#the-routing-fee) applied, it was reserved against your cap on top of that amount, and four fields on the record say so:

| Field | What it is |
| --- | --- |
| `feeAtomic` | The fee in the settlement asset's smallest unit — the same unit as `amount`. `0` when no fee applied. |
...
| `feeCents` | The cents this fee added to the cap reserve. |
| `feeStatus` | The fee's own lifecycle, tracked independently of the record's `status`. |

The fee is **configured per deployment and is currently `0`**, so today every row reads `feeBps: 0`, `feeStatus: "None"`, and a zero fee. Records created before the fee existed carry `null` in all four.
...
The CSV carries the same fields as the JSON, and the four fee columns sit at the **end** of the header row — so a consumer parsing by column index is unaffected by their arrival.

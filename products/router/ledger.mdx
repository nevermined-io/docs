...
The routing fee is **5%** (`feeBps: 500`). A row reads `feeStatus: "None"` and a zero fee where no fee applied — an MPP payment, or a mode-A call made without a `requestId`. Records created before the fee existed carry `null` in each of these fields.

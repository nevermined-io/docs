...
| `assetSymbol` | The ticker to display — `USDC`, `EURC`, `USDC.e`, `pathUSD` — resolved from `asset` on this row's `network`. `null` when it isn't an asset recognised there. |
| `assetDecimals` | The decimal scale of `amount` for this asset, so you never have to assume one. `null` when the asset is unrecognised. |
...
The CSV carries the same fields as the JSON. New columns are only ever **appended** to the header row — the fee columns first, then `assetSymbol` and `assetDecimals` — so a consumer parsing by column index is unaffected by their arrival.

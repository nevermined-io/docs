...
| --- | --- | --- |
| **x402** | The service answers `HTTP 402` advertising an `accepts` array (in the body for v1, in a `PAYMENT-REQUIRED` header for v2) | Supported — see [the x402 rail](/products/router/rails-x402) |
| **MPP** | The service answers `HTTP 402` with `WWW-Authenticate: Payment …` | Supported — see [the MPP rail](/products/router/rails-mpp) |
...
- **A Nevermined routing fee**, where one applies. Added **on top**, so your Delegation cap is debited the merchant's price *plus* the fee, while the merchant still receives their full price.

Every payment response carries a `fee` object saying exactly what was charged, and `fee.capChargedCents` — not `settlement.approxCents` — is the figure your budget actually moved by. See [the routing fee](/products/router/how-it-works#the-routing-fee) for how it is applied and rounded, and [the ledger](/products/router/ledger#the-routing-fee-on-a-record) for reading it back per payment.

<Note>
Read `fee.bps` from the response rather than assuming a rate: it is resolved per payment and per deployment, so it is the authoritative answer for the environment you are calling.
</Note>
...
<Steps>
  <Step title="A Nevermined API key">
    Sent as `Authorization: Bearer <your-api-key>` on every Router call. It must be a current-generation key — see [the quickstart](/products/router/quickstart) for how to tell.
  </Step>

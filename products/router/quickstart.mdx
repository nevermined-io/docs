...
<Note>
**Funding is manual today.** There is no on-ramp — you cannot top this wallet up from a card or from a Nevermined credit balance, and the agent cannot refill itself. When the wallet runs dry, payments stop with `402 BCK.ROUTER.0009` until a human sends more. Budget for that when you decide how much to fund: an agent is autonomous *after* it is funded, and only until the balance runs out.
</Note>

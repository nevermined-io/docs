...
```bash
cd .claude/skills/nvm-router-demo
./demo.sh --dry-run              # discovery and the plan; spends nothing
CONFIRM_LIVE=yes ./preflight.sh  # every precondition, ~20s
CONFIRM_LIVE=yes ./demo.sh       # the run, ~$0.08
```

<Warning>
**This spends real money.** There is no testnet variant, and that's structural rather than an oversight: the catalog lists mainnet merchants, so a testnet run would have nothing real to discover. Both `preflight.sh` and `demo.sh` refuse to touch a live key without `CONFIRM_LIVE=yes`.
</Warning>

Run `./preflight.sh` first, always. It resolves your wallet address from the API rather than from anything you wrote down, checks the balance on **both** chains, confirms each merchant still answers `402` on the rail you expect, and verifies the catalog is actually loaded rather than serving placeholder rows. Every one of those has caused a failed run that looked like a Router bug.

...
<Warning>
**Why splits are refused.** An MPP `charge` can name a primary recipient *and* a list of additional payout recipients. Your Delegation's recipient scope authorizes the primary one — so honouring splits would move real funds to addresses nobody validated. The Router therefore rejects any split-bearing challenge rather than paying the part it can vouch for.
</Warning>

...
On **x402 there is no balance check anywhere.** The Router signs and reserves your budget, and the shortfall surfaces only when the transfer is attempted on-chain — the payment lands as `Failed` and the reserved **merchant** amount is **not** given back. Only a [routing fee](/products/router/how-it-works#when-a-fee-is-not-collected) reserved on top of it is ever credited back, and only when the fee provably wasn't collected. Fund for the merchant's amount **plus** any [routing fee](/products/router/how-it-works#fund-for-the-amount-plus-the-fee), which is a second transfer from the same wallet.

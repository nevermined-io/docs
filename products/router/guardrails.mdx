...
For crypto-funded Delegations the Router enforces **recipient scope**: the merchant's pay-to address must be one the Delegation permits. This is checked *before* the expensive signing step, so a disallowed recipient costs you nothing.

On the MPP rail this extends to a refusal worth calling out: a challenge carrying **`splits`** — additional payout recipients beyond the primary one — is rejected outright. Recipient scope authorizes the primary recipient, so honouring splits would move real money to addresses nobody validated. The Router refuses the whole challenge rather than paying the part it can vouch for.

Card-funded Delegations work differently by design: they're vendor-agnostic with no recipient allowlist, so the cap and the expiry are the guardrails.

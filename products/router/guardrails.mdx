...
On **x402 there is no balance check anywhere.** The Router signs and reserves your budget, and the shortfall surfaces only when the transfer is attempted on-chain — the payment lands as `Failed` and the reserved cap is **not** given back. Fund for the merchant's amount **plus** any [routing fee](/products/router/how-it-works#fund-for-the-amount-plus-the-fee), which is a second transfer from the same wallet.

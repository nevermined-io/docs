...
<Warning>
**How an underfunded wallet fails depends on the rail.** On **MPP** the Router reads your balance *before* signing, so a short wallet costs you nothing — you get `402 BCK.ROUTER.0009`, no credential is minted and no budget is reserved.

On **x402 there is no balance pre-check.** The Router signs, your budget is reserved, and the shortfall only surfaces when the transfer is attempted on-chain — at which point the reserved cap is **not** given back. Fund the wallet before you call; don't rely on being stopped.
</Warning>

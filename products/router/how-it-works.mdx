...
<Note>
**The fee is configured per deployment, and it is currently `0` in both environments documented here** — `api.sandbox.nevermined.app` and `api.live.nevermined.app`. Every payment against those reserves exactly the merchant's amount and comes back with `fee.bps: 0`. The fields below are already on the wire so your reconciliation doesn't have to change on the day a rate is set.

Because it is per deployment, do not treat that as a property of the Router: read `fee.bps` off the response for the environment you are actually calling.
</Note>

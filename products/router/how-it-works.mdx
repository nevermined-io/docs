...
<Warning>
**On x402 an underfunded wallet costs you budget.** With no pre-check the Router signs and reserves your cap, the merchant's transfer then fails on-chain, and the reserved **merchant** amount is **not** given back. Only a [routing fee](#when-a-fee-is-not-collected) reserved on top can come back, and only when the fee provably wasn't collected. Fund before you call; don't rely on being stopped.
</Warning>
...
A reserved fee isn't always collected, and the two legs of the reserve are treated differently when it isn't.

**The merchant portion is never given back.** Once the credential is minted, the merchant may have taken the payment and then errored — so the reservation stands whatever the call did afterwards.

**The fee portion is given back whenever the fee provably wasn't collected.** A mode-B hop that never returned 2xx, a facilitator that adjudicated and refused, a fee movement that couldn't be signed: on each of those the fee's cents are credited back to your Delegation cap, and the ledger row reads [`feeStatus: "Released"`](/products/router/ledger#fee-lifecycle). What is *not* released is a fee whose outcome is unknown — money may have moved, so those rows stay `Submitted` and are resolved against the chain instead.

The mode decides when that question is even asked. Mode B settles the fee only after the merchant returns 2xx, so a hop that fails releases the fee. Mode A settles it at mint, before your credential is ever presented — so on mode A a wallet holding only the merchant's price typically pays the fee and then loses the merchant leg on-chain, and there is nothing to release.

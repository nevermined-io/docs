...
WWW-Authenticate: Payment id="…", realm="api.nevermined.app", method="nevermined",
...
```bash
curl -X POST https://api.nevermined.app/api/v1/mpp/challenge \
...
```json
{
  "challenge": "Payment id=\"k7Hs…\", realm=\"api.nevermined.app\", method=\"nevermined\", …",
...
```bash
curl -X POST https://api.nevermined.app/api/v1/mpp/settle \
  -H "Authorization: Bearer $NVM_API_KEY" \
  -H "content-type: application/json" \
  -d '{
...
| --- | --- |
| **Sandbox** | `https://api.sandbox.nevermined.dev` |
...
- Binding a challenge to a specific request body

...
**Price it first (optional): `POST /api/v1/router/quote`** — the same body minus `requestId`
(`delegationId` optional). Nothing is signed, minted, recorded or reserved, but the Router **does**
...
instead of paid; a rise within the same cent is still paid. It checks neither your cap nor your
wallet. An OAuth `commerce` key quotes on `POST /router/commerce/quote` (no `delegationId`).
...
a `feeStatus: "None"` row, `fee-not-quoted: <why>` when a rate was configured and none was quoted at
all. Human-facing diagnostic text, never a contract: branch on `feeStatus`.

# Nevermined — GitHub Copilot Instructions
...
- **Price it first: `POST /api/v1/router/quote`** — the same body minus `requestId` (`delegationId` optional). Nothing is signed, minted, recorded or reserved, but the Router **does** send the unpaid request to the service, so a service that does not charge for it performs it, and a quote spends the same per-key and per-service rate budgets as a payment: quote once per decision, don't poll. It returns `optionSet` (`delegation` when you named one, else `deployment` — what a personal crypto Delegation would select), `protocol`, `settlement` and `fee`. Pay with `maxTotalCents` set to its `fee.capChargedCents` — whole cents rounded **up** from `fee.capChargedMicros` (1/10,000 of a cent) — so a price that rose in between is refused (`402 BCK.ROUTER.0018`) instead of paid; a rise within the same cent is still paid. It checks neither your remaining cap nor your wallet balance. An OAuth `commerce` key quotes on `POST /api/v1/router/commerce/quote` instead, which prices the grant's Delegation and refuses a `delegationId` (`400 BCK.OAUTH.0034`).
...
| `BCK.OAUTH.0030` | 403 | The API key was OAuth-minted; it may not create Delegations or use `/router/{payments,route,proxy,svc}`. Use a plain account-owner key — or, for a `commerce` grant, `POST /router/commerce/route` to pay and `/router/commerce/quote` to price | No |

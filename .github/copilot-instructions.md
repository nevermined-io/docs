...
| `BCK.ROUTER.0025` | 502 | The upstream reply was too large to deliver after a paid request. The payment outcome is indeterminate; it may have gone through. | No — this reply has no `X-Router-Payment-Id`. Find the payment by `requestId` in `GET /api/v1/router/payments` and inspect `status` / `merchantSettlementObservedAt`. Do **not** retry with a fresh `requestId`; reuse the same one if retrying. |

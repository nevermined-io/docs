...
For tool-time gating inside an agent (`@requires_payment` decorator), see [LangChain Integration](/api-reference/python/langchain-module). The two integrations are complementary — that page is for protecting tools the agent calls; this page is for protecting the agent's HTTP entry point.
...
- [x402 Protocol](/api-reference/python/x402-module) for envelope and header semantics.
- [LangChain Integration](/api-reference/python/langchain-module) for the `@requires_payment` decorator (tool-time gating).

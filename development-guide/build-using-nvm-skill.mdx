...
The IDE rule files also omit `0007`. **Windsurf's copy additionally omits `BCK.ROUTER.0012` and `BCK.ROUTER.0014`**, because Windsurf truncates a rule file at 6,000 characters and drops the *end* — where the guardrails live — so the file cannot grow without displacing something. `0012` (a 400) and `0014` (a 409) are both non-retryable — a verdict the file still states as a rule ("only `0006`, `0007` and `0020` are retryable"); `BCK.ROUTER.0013` is kept because it is a **500**, the one shape an agent retries by default. Cursor, Cline and Amazon Q carry all three.
</Note>

...
The IDE rule files also omit `0007`. **Windsurf's copy additionally omits `BCK.ROUTER.0012`, `BCK.ROUTER.0014`, `BCK.ROUTER.0021`, `BCK.ROUTER.0026` and `BCK.ROUTER.0027`** to stay below its 6,000-character rule limit. These codes are non-retryable; the file also states that only `0006`, `0007` and `0020` are retryable. `BCK.ROUTER.0013` stays because its **500** status could otherwise invite a retry. For the specific remedies to omitted codes, use the complete Router skill. Cursor, Cline and Amazon Q include them.
</Note>

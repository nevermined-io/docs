...
| **`nevermined-router`** | **spending** money at external services | Discovering services in the catalog, creating a spending Delegation, paying any x402 or MPP endpoint through the [Nevermined Router](/products/catalog/router/overview), and the guardrails an autonomous buyer must respect |
...
<Note>
The Copilot and Codex files are **condensed** — they carry the buy loop, the money traps and every `BCK.ROUTER.*` retry decision, but not the full reference set: they omit `0004`/`0005` and `0030` (reads of a record, with nothing to retry), `0022` (a deployment/config fault, never seen in a healthy environment) and `0023` (the autoPay `/router/select` “charge indeterminate” outcome — whose reconcile-and-reuse-`requestId` remedy the `route_by_intent` tool result delivers at runtime, and which the `0020` row already models), everything the Router refuses outright (MPP splits, internal targets, redirects, forged `X-Router-*`), and the relay limits. For the complete Router skill, install it as a directory (Claude Code plugin, ClawHub, or `.agents/skills/`).

The IDE rule files also omit `0007`. **Windsurf's copy additionally omits `BCK.ROUTER.0012`, `BCK.ROUTER.0014`, `BCK.ROUTER.0021` and `BCK.ROUTER.0026`**, because Windsurf truncates a rule file at 6,000 characters and drops the *end* — where the guardrails live — so the file cannot grow without displacing something. `0012` (a 400), `0014` (a 409), `0021` (a 400) and `0026` (a 415) are all non-retryable — a verdict the file still states as a rule ("only `0006`, `0007` and `0020` are retryable"); `BCK.ROUTER.0013` is kept because it is a **500**, the one shape an agent retries by default. What a Windsurf reader loses is each code's specific remedy — for `0021`, that the fix is to name the credential's header via `credentialHeader` / `X-Router-Credential-Header` rather than to stop; for `0026`, that the body must be resent as JSON or a URL-encoded form. Cursor, Cline and Amazon Q carry all five.
</Note>

...
A plan is either **crypto** (`nvm:erc4337`) or **fiat** (`nvm:card-delegation`) — never both — and the buyer's setup has to match the plan. Most rejections come from that mismatch. First [detect the plan's type](/development-guide/order-plans#which-payment-type-does-this-plan-need), then:

| What you see | Likely cause | Fix |
|---|---|---|
| `BCK.X402.0027` — "No accessible payment method found…" (or CLI "No enrolled card found") | Fiat plan, but no card is enrolled | [Enroll a card](/products/payments/card-enrollment) with the plan's provider, create a card delegation, then retry |
...
See [Automatic Credit Top-Ups](/integrate/patterns/top-up#when-top-ups-stop) for how funding and delegation-limit failures surface (402 vs 500).

...
The `nvm:erc4337` scheme uses ERC-4337 smart accounts instead of EOA wallets, enabling programmable settlement through session keys and UserOperations.
...
<Tabs>
  <Tab title="TypeScript">
    ...
    if (response.status === 402) {
      ...
      // 1. Create a delegation once. `provider` and `currency` are required:
      //    'usdc' for the crypto (erc4337) scheme, 'usd' for card providers.
...
<Tip>
With a delegation in place you can skip the manual `order_plan()` / `orderPlan()` step entirely. If the subscriber's balance is short when a request settles, the facilitator tops up their credits automatically, up to the delegation's limit. See [Automatic Credit Top-Ups](/integrate/patterns/top-up).
</Tip>
...
<Tabs>
  <Tab title="Express.js (TypeScript)">
    ...
    See the [Express.js Integration Guide](/integrate/add-to-your-agent/express) for full details.
  </Tab>
  <Tab title="FastAPI (Python)">
    For FastAPI applications, use the `PaymentMiddleware` from `payments_py.x402.fastapi`:
    ...
    See the [FastAPI Integration Guide](/integrate/add-to-your-agent/fastapi) for full details.
  </Tab>
</Tabs>
...
the `scheme` parameter in the route configuration.
</Note>
...
<Tabs>
  <Tab title="TypeScript">
    ...
    // Reuse the delegation you created for x402 (see "Build a payment payload" above).
    const { response, settled, receipt } = await payments.mpp.fetch(
      'https://api.example.com/ask',
      { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt: 'Hello' }) },
      { delegationConfig: { delegationId: delegation.delegationId }, planId: PLAN_ID },
    )

    if (settled) {
      console.log('Paid. Receipt reference:', receipt?.reference)
  ...
  <Tab title="Python">
    ...
    result = payments.mpp.fetch(
        ...
        MppFetchOptions(
            ...
            max_credits="10",       # optional: cap the credits for the whole call
        ),
        json={"prompt": "Hello"},
    )

    if result.settled:
        print("Paid. Receipt reference:", result.receipt)
...
<Steps>
  ...
  <Step title="Mint an MPP access token for that plan">
    ...
    ```json
    {
      ...
      "accepted": { "scheme": "nvm:erc4337", "network": "<network-id>", "planId": "<from the challenge>" },
...
- **Express.js and FastAPI middleware** handle the entire flow automatically (and accept MPP with a one-key opt-in)
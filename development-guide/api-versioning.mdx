...
- An unknown or out-of-range value is rejected with `400` and a stable, machine-readable [API error code](/development-guide/api-errors/overview) (`BCK.VERSION.0001`).
...
A version only appears in `gatedVersions` when it changed the wire shape — `GET /api/v1/meta/versions` reports the current set, and each gate translates the difference for older pins.
...
1. Check which versions between your pin and the target actually changed the wire shape — the `gatedVersions` list from `GET /api/v1/meta/versions`.

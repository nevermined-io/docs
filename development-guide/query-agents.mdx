---
title: "Query Agents"
description: "Query paid AI agents over HTTP. Get an x402 access token and send it in the payment-signature header to authorize calls with the Nevermined Payments SDK."
...
<Note>
  Is this a **fiat** plan or a **crypto** plan? It determines the `provider`/`currency` above and whether you must pass a `scheme`. The direct `getX402AccessToken` call defaults to crypto (`nvm:erc4337`) — for card plans pass `scheme: 'nvm:card-delegation'`. See [Order Plans](/development-guide/order-plans#which-payment-type-does-this-plan-need) for how to detect a plan's type before you pay.
</Note>
...
<CardGroup cols={2}>
  <Card title="Build AI Agents" icon="robot" href="/development-guide/process-requests">

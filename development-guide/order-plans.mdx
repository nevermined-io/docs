...
description: "Purchase Nevermined payment plans to access AI agents. Pay in crypto (USDC, EURC, or any ERC-20) or via fiat checkout with Stripe or Braintree."
icon: "credit-card"
---
...
| **Fiat** | `nvm:card-delegation` | An [enrolled card](/products/payments/card-enrollment) with the plan's provider (Stripe, Braintree, or Visa) |
...
<Note>
  Server-side middleware (Express, FastAPI) and the A2A clients resolve the scheme for you. The direct `getX402AccessToken` / `get_x402_access_token` call does **not** — it defaults to `nvm:erc4337`. For a fiat plan you must pass `scheme: 'nvm:card-delegation'` and have a card enrolled with the matching provider. See [Core Concepts](/getting-started/core-concepts#fiat-vs-crypto-what-each-plan-needs) for the full prerequisites and [Payment method mismatch](/development-guide/integration-faq#how-do-i-fix-a-payment-method-mismatch) if a payment is rejected.
</Note>
...
<CardGroup cols={2}>
  <Card title="Query AI Agents" icon="search" href="/development-guide/query-agents">
    Learn how to access AI services with purchased plans
  </Card>
  <Card title="Handle Requests" icon="chart-line" href="/development-guide/process-requests">

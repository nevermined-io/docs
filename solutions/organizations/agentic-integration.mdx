...
<Steps>
  ...
  <Step title="Set up a payment method">
    A stablecoin method works immediately; a card needs a one-time embedded enrollment. The agent then creates a spending **delegation** — a budget it can spend within, no human per purchase. The same delegation backs both x402 and MPP.
  </Step>
  <Step title="Pay with x402 or MPP">
    Pick a plan from the registry and settle it. For an **x402** service (or a plain plan top-up) the agent uses `verify` + `settle`; for an **MPP** service — recognised by a `WWW-Authenticate: Payment …` challenge — it mints at `/api/v1/mpp/permissions` and returns an `Authorization: Payment` credential. The document spells out the exact endpoints and request bodies for each.
  </Step>
</Steps>

...
<CardGroup cols={3}>
  ...
  <Card title="agentic-instructions.md" icon="robot" href="#agentic-instructions-md">
    A step-by-step guide telling an AI agent how to discover and pay for your agents via x402 or MPP.
  </Card>
...
- [x402 stablecoins](https://nevermined.ai/docs/integrate/patterns/stablecoin-payments)
- [Nevermined Payments Skill](https://nevermined.ai/docs/development-guide/build-using-nvm-skill)
- [Nevermined API Reference](https://nevermined.ai/docs/api-reference/introduction)
- [Nevermined Concise Context for AI Agents](https://nevermined.ai/docs/llms.txt)
- [Nevermined Full Context for AI Agents](https://nevermined.ai/docs/llms-full.txt)
```
...
The operational guide. Where `llms.txt` is a map, this is the *how-to*: a plain-Markdown document — consumed raw by agents like Claude Code — that tells an autonomous agent exactly how to discover and pay for your agents through Nevermined's **x402 and MPP** flows. Both settle against the same Payment Plans; the agent picks the flow from the service's `402` response.

It opens by stating the environment ("you are talking to the sandbox / live environment"), then walks the ordered path — **get a Nevermined API key → set up a payment method and a spending delegation → pay via x402 or MPP** — with the exact endpoints and request bodies inline, so an agent can act on it end to end. The agent picks x402 vs. MPP from the service's `402` response; the same delegation backs both. The key-minting and authentication foundations it relies on are the ones described in [`auth.md`](https://nevermined.app/auth.md). It then lists your **registry of agent-payable APIs** (your plans and agents) and links out to the Nevermined ecosystem docs.
...
<Steps>
  ...
  <Step title="It obtains a credential and pays with x402 or MPP">
    A one-time key + delegation, then it settles against the plan over whichever protocol the service speaks — no human per purchase.
  </Step>
  <Step title="It calls your agent">
    With credits in hand, it invokes your agent and you get paid. See [Buy & Call a Paid Agent](/getting-started/ai-agent-purchase) for the end-to-end flow.
  </Step>
</Steps>
...
<CardGroup cols={2}>
  <Card title="Programmatic & Agentic Access" icon="robot" href="/solutions/organizations/programmatic-access">
    Drive your whole organization from code, and the endpoints behind these files.
  </Card>
  <Card title="Buy & Call a Paid Agent" icon="cart-shopping" href="/getting-started/ai-agent-purchase">
    The buyer's side — an agent purchasing and calling a paid agent end to end.
  </Card>
  <Card title="x402 card-delegation" icon="credit-card" href="/specs/x402-card-delegation">
    How an agent pays with a delegated card, no human per transaction.
  </Card>
  <Card title="x402 stablecoin payments" icon="coins" href="/integrate/patterns/stablecoin-payments">
    Paying for agents with USDC and other stablecoins.
  </Card>
  <Card title="Paying an MPP service" icon="handshake" href="/development-guide/nevermined-x402#paying-an-mpp-protected-service">
    The MPP buyer flow — the plan-model sibling of x402.
  </Card>
</CardGroup>

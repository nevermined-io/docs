...
<Note>
  Onboarding customers is an **Admin** operation — you call it with your organization's admin API key. The customers you onboard appear in your [Customers](/solutions/organizations/customers) CRM (an Enterprise feature).
</Note>
...
<CardGroup cols={2}>
  <Card title="Members" icon="users" href="/solutions/organizations/workspaces-and-members">
...
It's also **short-lived** and **revocable**, so a leaked customer credential has a small blast radius. The exact expiry is returned as `expiresAt` (ISO-8601) — track it and re-onboard to refresh before it lapses. Store it server-side and never expose it to the browser.

<Note>
  **Using the raw REST API instead of the SDK?** `POST /organizations/account` returns **two** similar-looking credential fields — present `walletResult.hash` as the `Authorization: Bearer` token; that is the credential (the dashboard copies exactly this). The sibling `walletResult.nvmApiKey` is the encrypted server-side/SDK blob — **do not send it as a header**; it is not a valid Bearer. The SDK handles this for you: `onboardCustomer`'s `nvmApiKey` result field already carries the Bearer, so pass it straight to `Payments.getInstance`.
</Note>
...
From there, the `customer` client behaves like any buyer: it can [order a plan](/getting-started/ai-agent-purchase) and [pay your agents over x402](/getting-started/ai-agent-purchase) — all attributed to your organization, all invisible to the end user.
...
<CardGroup cols={2}>
  ...
  <Card title="Embedded widgets" icon="puzzle-piece" href="/integrations/organization-widgets">
...
<CardGroup cols={2}>
  ...
  <Card title="Organization widgets" icon="puzzle-piece" href="/integrations/organization-widgets">
    The embedded, UI-driven counterpart to programmatic onboarding.
  </Card>
  <Card title="Purchase & pay an agent" icon="cart-shopping" href="/getting-started/ai-agent-purchase">
    What the customer credential does next: order a plan and pay over x402.
  </Card>
  <Card title="Activity & Events" icon="bolt" href="/solutions/organizations/activity-and-events">

...
<Note>
  **Using the raw REST API instead of the SDK?** `POST /organizations/account` returns **two** fields — present `walletResult.hash` as the `Authorization: Bearer` token; that is the credential (the dashboard copies exactly this). The sibling `walletResult.nvmApiKey` is the encrypted server-side/SDK blob — **do not send it as a header**; it is not a valid Bearer. The SDK handles this for you: `onboardCustomer`'s `nvmApiKey` result field already carries the Bearer, so pass it straight to `Payments.getInstance`.
</Note>

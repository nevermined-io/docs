...
[**Baselayer**](https://baselayer.com) shows the metered variant. Its plan is a `credits` plan rather than pay-as-you-go: a \$10 purchase mints 100 credits, and every KYB business search an agent runs burns 10 of them, so the card is charged once per ten requests instead of once per key. The agent presents the same x402 token on every call, and Baselayer verifies and settles each one — the first call provisions the API key, the rest are searches.

...
- You.com integration guide: [/integrations/youdotcom](/integrations/youdotcom)

[**Baselayer**](https://baselayer.com) shows the metered variant. Its plan is a `credits` plan rather than pay-as-you-go: a \$10 purchase mints 100 credits, and every request an agent makes — the key purchase included — burns 10 of them, so the card is charged once per ten requests instead of once per key. The agent presents the same x402 token on every call, and Baselayer verifies and settles each one — the first call provisions the API key, the next nine are KYB business searches. It also publishes the agent-facing `.md` mirror described below, listed in its own `llms.txt`.

- Baselayer integration guide: [/integrations/baselayer](/integrations/baselayer)
...
<Tabs>
  ...
  <Tab title="HTTP (any language)">
    ...
    **Settle** uses the same body shape posted to `/x402/settle`. Response
    contains `billingModel`, `creditsRedeemed`, `remainingBalance`,
    `transaction`, and `orderTx`. Read `billingModel` before the credit
    fields: on a `pay-as-you-go` plan both read `"0"` even on a charge that
    succeeded, and the charge is referenced by `orderTx` (fiat) or
    `transaction` (crypto). On a `credits` plan `orderTx` is present only when
    auto top-up fired.
    ...
    **Pseudo-handler** (curl-style):
    ...
    See the [Verify](/api-reference/x402/verify-permission) and
    [Settle](/api-reference/x402/settle-permission) API references for
    full request/response schemas, and the
    [card-delegation spec](/specs/x402-card-delegation) §3.2 for the
...
<Note>
  ...
  [card-delegation spec](/specs/x402-card-delegation) §4.3. The optional
...
The middleware reads the `payment-signature` header, calls `verifyPermissions`, runs your handler, then calls `settlePermissions` and attaches the `payment-response` header before sending the response. See the [Express](/integrate/add-to-your-agent/express) and [FastAPI](/integrate/add-to-your-agent/fastapi) integration pages for middleware options (dynamic credits, hooks, custom error handlers).
...
<AccordionGroup>
  ...
  <Accordion title="What card rails are supported?">
    ...
    - **[Stripe](/products/payments/overview)** — direct card enrollment
      and Stripe PaymentIntents settlement
    - **[Braintree](/products/payments/braintree-onboarding)** — Drop-in
    ...
    [card-delegation spec](/specs/x402-card-delegation) for the
    underlying flow.
  </Accordion>
  ...
  <Accordion title="What languages and frameworks are supported?">
    ...
    [Generic HTTP guide](/integrate/add-to-your-agent/generic-http).
  </Accordion>
  ...
  <Accordion title="How are chargebacks handled?">
    Even though you are the merchant of record, Stripe debits **Nevermined's**
    balance for disputes on these charges — so chargeback exposure sits with
    us, not with you. We surface the chargeback or charge reference so you can
    match a dispute back to the specific agent transaction that caused it.
  </Accordion>
...
Every account starts on the free **Personal** tier. Upgrading that workspace to a paid organization tier — **Premium** or **Enterprise** — happens from the [Billing page](https://nevermined.app/organization), and you can change tier later.
...
<Warning>
  **A subscription covers one environment only.** Upgrading your **sandbox**
  workspace does not upgrade **live**, and vice versa — so a provider running
  both is subscribing twice. Trial a tier on sandbox, then subscribe
  separately on live. See [Plans & Billing](/solutions/organizations/plans-and-billing).
</Warning>

Prices above are indicative; the live price for each tier is shown on the upgrade screen. Settlement fees on agent payments are documented in the [Payments overview](/products/payments/overview).
...
- [Manage Plans](/products/nevermined-app/manage-plans) - set up a
  pay-as-you-go plan
- [Payments overview](/products/payments/overview) - how buyers
...
- [Platform Partners (Organizations)](/integrations/organizations) - the
  recommended org path with widgets, analytics, and webhooks

...
<Note>
  This is a **seller** task. If you only want to **buy or use** an agent, you don't register one — see [Buy & Call a Paid Agent](/getting-started/ai-agent-purchase).
</Note>
...
<Steps>
  ...
  <Step title="Register the agent and plan together">
    ...
    <Accordion title="URL pattern syntax (path-to-regexp)">
      Endpoint URLs are matched with [path-to-regexp](https://github.com/pillarjs/path-to-regexp), so one entry can cover a whole family of routes:
      ...
      Use `openEndpoints` (TypeScript) or `open_endpoints` (Python) for routes that must stay public, such as documentation or health checks.
    </Accordion>
  </Step>
...
<Note>
  Prefer no code? You can register and price everything in the web app — see [Register Agents](/products/nevermined-app/register-agents).
</Note>

## Related

<CardGroup cols={2}>
  <Card title="Add payments to your agent" icon="plug" href="/agents-guide/add-payments">
    Gate the registered agent's endpoints behind the plan.
  </Card>
  <Card title="Register Agents (App)" icon="store" href="/products/nevermined-app/register-agents">
    The no-code path to publishing agents and plans.
  </Card>
  <Card title="Check revenue" icon="chart-line" href="/agents-guide/check-revenue">
    Track sales once buyers start arriving.
  </Card>
  <Card title="Order Plans" icon="cart-shopping" href="/getting-started/ai-agent-purchase">

...
<Steps>
  ...
  <Step title="Option B — manual paste (works anywhere)">
    If the agent can't host a localhost callback, the human creates the key in the app and pastes it back: sign in at [nevermined.app](https://nevermined.app) → **Settings → Global NVM API Keys** → create a key for the right environment.
  </Step>
...
<Note>
**Two cases need neither option.** A **headless** agent with no browser and no localhost can still get a key with a human's approval through the [device flow (RFC 8628)](/integrate/authentication/device-flow). And an agent running **inside an AI assistant** that runs OAuth for it — Claude, ChatGPT, and the other hosts Nevermined's operator has listed — never handles a key at all: the assistant connects the [Catalog MCP](/products/catalog/mcp#auth-&-transport) and obtains one when the human approves a spending cap (which host and client each assistant uses differs — LangSmith Fleet, for one, is a pre-registered client on the strict host). Both ceremonies, the per-assistant mapping, and the key format are in the agent-facing manifest [`auth.md`](https://nevermined.app/auth.md).
</Note>

## Related

<CardGroup cols={2}>
  <Card title="Buy access" icon="cart-shopping" href="/getting-started/ai-agent-purchase">
    With a key in hand, make your first autonomous purchase.
  </Card>
  <Card title="Register a plan and an agent" icon="rocket-launch" href="/agents-guide/register-plan-and-agent">
    Publish a service others can pay for.
  </Card>
</CardGroup>

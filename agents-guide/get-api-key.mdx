...
<Steps>
  ...
  <Step title="Option B — manual paste (works anywhere)">
    If the agent can't host a localhost callback, the human creates the key in the app and pastes it back: sign in at [nevermined.app](https://nevermined.app) → **Settings → Global NVM API Keys** → create a key for the right environment. See [Get Your API Key](/getting-started/get-your-api-key) for the full UI walkthrough.
  </Step>

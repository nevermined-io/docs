...
<Note>
These endpoints are browser-initiated: the subject comes from the authenticated session, so a caller only ever sees and manages **their own** bindings. Requires a Nevermined API key.
</Note>

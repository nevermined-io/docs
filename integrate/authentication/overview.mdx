...
<CardGroup cols={3}>
  ...
  <Card title="Device flow (RFC 8628)" icon="mobile-screen">
    You're a headless agent with no browser, but a human can approve for you. Ends with an **NVM API key** bound to the approver. See [Device flow](/docs/integrate/authentication/device-flow).
  </Card>
  <Card title="Authorization Code + PKCE" icon="browser">
    You're a browser-based MCP client (Cursor, Claude) connecting to a paid agent. Mints an **x402 payment permission** for a specific agent and plan. See [Authorization Code](/docs/integrate/authentication/oauth-authorization-code).
  </Card>
</CardGroup>

...
<CardGroup cols={3}>
  ...
  <Card title="Device flow (RFC 8628)" icon="mobile-screen">
    You're a headless agent with no browser, but a human can approve for you. Ends with an **NVM API key** bound to the approver. See [Device flow](/integrate/authentication/device-flow).
  </Card>
  <Card title="Authorization Code + PKCE" icon="browser">
    You're a browser-based MCP client (Cursor, Claude) connecting to a paid agent. Mints an **x402 payment permission** for a specific agent and plan. See [Authorization Code](/integrate/authentication/oauth-authorization-code).
  </Card>
</CardGroup>

<Note>
The two OAuth grants mint **different credentials**. Authorization Code + PKCE yields an *x402 payment permission* for an agent+plan. The RFC 8628 device flow yields the *NVM API key* that authenticates you against this API. Pick the flow by the credential you actually need.
</Note>
...
| JWKS (ES256K / secp256k1) | `/.well-known/jwks.json` | RFC 8812 |
...
Once a card is enrolled, spending is yours to drive with no further browser step — create a delegation and reuse its id on your x402 requests.

...
<Steps>
  ...
  <Step title="Handle errors by code">
    Most failures return a typed `BCK.<MODULE>.<NNNN>` code, a `message`, often a `hint`, and a `category`. On a `401`, branch on `category`: `auth` means fix your key; `business`/`integration` means you reached something that isn't yours. The `x-correlation-id` header is always present. See the [error codes](/docs/development-guide/api-errors/codes).
  </Step>
</Steps>

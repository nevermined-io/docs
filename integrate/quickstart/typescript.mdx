...
```bash
# Your Nevermined API key (see: /docs/agents-guide/get-api-key)
...
async function purchaseAndQuery() {
  // 1. Order the plan (buys credits up front)
  ...
  // Create the delegation first (provider + currency required), then request
  // the token by its delegationId. Reuse the delegation for later requests.
...
<Tip>
For agents that run continuously, set up a delegation when generating the access token — the facilitator tops up credits automatically when the balance runs out, with no manual `orderPlan()` loop needed. See [Automatic Credit Top-Ups](/integrate/patterns/top-up).
</Tip>
...
<CardGroup cols={2}>
  <Card title="Express Integration" icon="node-js" href="/integrate/add-to-your-agent/express">
    Complete Express.js patterns and best practices
  </Card>

  <Card title="TypeScript SDK Reference" icon="book" href="/api-reference/typescript/installation">
    Full API documentation
  </Card>

  <Card title="Payment Patterns" icon="code" href="/integrate/patterns/validate-requests">
    Advanced patterns for production
  </Card>

  <Card title="x402 Protocol" icon="credit-card" href="/development-guide/nevermined-x402">

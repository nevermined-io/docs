...
Accept credit cards in **USD or EUR**, with no crypto wallet required. Fiat payments are the fastest path to mainstream adoption. Your users can start paying with a card they already have, either through Stripe Checkout (one-time plan purchase) or by setting up [card delegation](/products/payments/overview) for autonomous agent charging.
...
- **Agent-to-Agent**: An agent owner enrolls a credit card (Visa, Stripe, or Braintree) and creates a [spending delegation](/products/payments/overview). The agent can then make autonomous payments on behalf of the owner, charging the card per request within the defined limits.
...
<CardGroup cols={2}>
  ...
  <Card title="Card Delegation" icon="key" href="/products/payments/overview">
...
Fully autonomous agent-to-agent payments in fiat are enabled through [card delegation](/products/payments/overview). This allows an agent to charge a user's card on-demand, without the user needing to pre-purchase credits. Instead of buying credits upfront, the user enrolls a card and creates a spending delegation with defined limits.
...
<Steps>
  <Step title="Card Enrollment">
    The user enrolls a card via the [Nevermined App](https://nevermined.app). Card data is tokenized by the provider (VGS Collect for Visa and Stripe; Braintree Drop-in for Braintree) and Nevermined never stores raw card numbers. See [Card Enrollment](/products/payments/card-enrollment) for the full flow.
  </Step>

  <Step title="Create a spending delegation">
    The user creates a delegation that defines spending limits: lifetime amount ceiling, max transactions, and duration. Optionally, they can link it to a specific API key for automatic selection. Visa delegations require an additional per-delegation passkey approval (browser-only). See [Delegations](/products/payments/mandates).
  </Step>
...
<Note>
Card delegation is what enables fiat-based agent-to-agent payments. An agent owner enrolls their card and sets spending limits, and the agent can autonomously pay for services from other agents, all settled in fiat. Credit top-ups are built in: if the balance runs out during a request, the facilitator charges the enrolled card within the delegation limits — see [Automatic Credit Top-Ups](/integrate/patterns/top-up). For common questions, see the [Payments FAQ](/products/payments/faq).
</Note>
...
Once a delegation leaves the **Active** state, a new one must be created to continue charging. See [Delegations](/products/payments/mandates) for full details on spending controls and the [Delegation Selection](/products/payments/mandate-selection) algorithm.
...
<Warning>
Fiat payments carry higher fees than stablecoin payments (2% + Stripe fees vs. 1% flat). For high-volume or high-value use cases, consider [stablecoin payments](/integrate/patterns/stablecoin-payments) to reduce costs.
</Warning>
...
<CardGroup cols={2}>
  <Card title="Payments" icon="credit-card" href="/products/payments/overview">
  ...
  <Card title="Orders: Goods & Services Checkout" icon="cart-shopping" href="/integrate/patterns/orders">
    Charge an arbitrary cart total by card in the browser, with no plan and no buyer account
  </Card>

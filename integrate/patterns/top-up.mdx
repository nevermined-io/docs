...
For card-delegation plans the top-up is built in: there's no extra parameter to pass — enroll a card, set its spending limit, and the facilitator charges it when a request would otherwise run dry. See [Fiat Payments](/integrate/patterns/fiat-payments) for card enrollment.

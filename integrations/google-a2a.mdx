...
After [purchasing access to the payment plan](/getting-started/ai-agent-purchase) associated with the AI agent, a client [can generate an access token](/development-guide/nevermined-x402) and start sending tasks:

<Note>
  The TypeScript example below targets a **crypto** plan. For a **fiat** plan, create the delegation with the card provider (`provider: 'stripe'`, `currency: 'usd'`) and pass `scheme: 'nvm:card-delegation'` to `getX402AccessToken` — or use the `PaymentsClient` (Python tab), which resolves the scheme for you. See [Which payment type does this plan need?](/getting-started/core-concepts#fiat-vs-crypto-what-each-plan-needs).
</Note>

...
<Note>
This is the **seller** side — you put a paywall on *your own* MCP server so its tools get paid. If instead you want an agent to **discover and pay for** Catalog services through our hosted MCP server, that's the buyer side — see the [Catalog MCP](/products/catalog/mcp).
</Note>
...
<Note>
**Which consent the human sees, and how the client knows who answered.** A client that reached the consent page through discovery never saw `?consent_type=` — the tier's `authorization_endpoint` carries only `network`. Such a client selects the consent with the standard `scope` parameter, which MCP clients forward from the server's `WWW-Authenticate` challenge: `scope=commerce` opens the **commerce** consent (an account-level spend cap the user funds from their Nevermined wallet, for paying external services through the Router — no `agent_id`), while a request that names an `agent_id` gets the agent-specific plan consent this page describes. An explicit `consent_type` on the URL is kept as long as `scope` does not contradict it, so a provider you configure by hand (LangSmith Fleet's Agent-Auth) keeps its `?consent_type=commerce`; a URL that contradicts itself (`consent_type=account_access&scope=commerce`, or `scope=commerce` with an `agent_id`) is refused rather than guessed. Nevermined publishes no `scopes_supported` in its own document — the API enforces no scope vocabulary; `scope` only picks the screen.

Whichever screen it was, the redirect back to the client — the `code` on approval, the `error` on denial — carries **`iss`**, the RFC 9207 issuer identifier, equal to the tier document's `issuer`; the document advertises it with `authorization_response_iss_parameter_supported: true`. A conforming client compares it with the issuer it discovered and rejects a mismatch. ChatGPT, for one, presents its stable `https://chatgpt.com/oauth/client.json` client identity only to an authorization server that does this — see [the overview](/integrate/authentication/overview#the-authorization-response-names-its-issuer-rfc-9207).
</Note>

...
<a id="pending"></a>
**A slow service answers `202` — the payment is made, the result comes later.** If the service has not answered within **45 s**, the Router stops holding your connection and returns `202 { "paymentId", "resultUrl", "status": "Pending" }` (on `/router/proxy` and `/router/svc`: `202` with `Location` and `X-Router-Payment-Id`). The call keeps running on the Router (up to its 120 s ceiling) and its fee is collected on a `2xx` or released otherwise, exactly as if you had waited. **Poll `GET $NVM_API_URL{resultUrl}`** with the same key: `202` while `Pending`, then `200` with `state: "Ready"` (`status` + `body`, as `/route` would have returned them) or `state: "Failed"` (`failureReason`). **Never pay again for a `202`** — re-sending the same `requestId` returns the same `202`, then the result. Every paid result is kept for 24 hours for the paying account only, so a response you lost can be read the same way from `payment.resultUrl`. Keys on API version 1.48 or later get this behaviour; older keys keep waiting.
...
<a id="requestid"></a>
**`requestId` is required, and it is an idempotency key — not a request counter.** Use **one stable id per logical purchase** and reuse it across retries of that purchase. Retrying a dropped call with the same id never buys twice: from API version 1.48 it returns the **retained result** of the first purchase (or its `202` while it is still running); older keys get `409 BCK.ROUTER.0002` carrying the original `paymentId` — read the result from `GET /router/payments/{paymentId}/result`. A fresh id buys twice, on purpose. **Never answer that 409 by minting a fresh id**: that is the double-spend the key just prevented. If the purchase genuinely failed, report it. Derive it from the work you are doing (`"search-nevermined-router-v1"`), not from `uuid4()` per HTTP attempt — a fresh UUID on every retry is how an agent double-spends.
...
| `BCK.ROUTER.0026` | 415 | Streaming surfaces only (`/proxy` · `/svc`): the request body cannot be forwarded. They forward a body only when it is JSON (`application/json`) or a URL-encoded form (`application/x-www-form-urlencoded`), and never on a `GET` or `HEAD`; anything else — a `multipart/form-data` file upload above all, but also `text/plain`, `application/octet-stream` or a vendor `+json` type — would have reached the service **empty**, so the Router refuses it instead. **Nothing was minted, no budget was reserved and no money moved.** JSON-string `params.contentType` names the request's content type (`null` if absent or malformed) | No — for a body on `GET`/`HEAD`, send none (or use `POST`/`PUT` if the service takes one) — changing the content type does not help; otherwise resend the body as JSON or a URL-encoded form if the service accepts one. A service that only takes a file upload cannot be paid through the Router yet. Retrying unchanged fails identically |
| `BCK.ROUTER.0030` | 404 | No retained result for that `paymentId` under your account: it expired (a paid result is kept **24 hours** after the call ends), it was never retained (a streamed response that had already started, or a payment not made through `/router/route`, `/router/proxy` or `/router/svc`), or it is not your payment — deliberately one answer. The payment record itself is unaffected (`GET /router/payments`). | No |

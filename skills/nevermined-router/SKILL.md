...
<a id="quote"></a>
**Check the price before you pay: `POST /api/v1/router/quote`.** Same body as `/route` minus `requestId` (`delegationId` optional). The Router makes the same unpaid probe, picks the option a payment would pick (MPP first), prices it with the fee — and stops: nothing is signed, minted, recorded or reserved. It answers `200` with `paymentRequired`, `optionSet` (`delegation` if you sent one, else `deployment`), `protocol`, and the same `settlement` and `fee` objects `/route` returns. Then route with `maxTotalCents` set to the quote's `fee.capChargedCents`, so a price that moved in between is refused (`402 BCK.ROUTER.0018`) instead of paid. A quote does not check your remaining cap or wallet balance — the payment still does.
...
| `BCK.ROUTER.0028` | 503 | `POST /router/quote` could not price the call because a chain read the price depends on failed (today: the MPP-tempo settlement token's `decimals`). A quote never signs, mints or charges anything, so **nothing was signed, reserved or charged**. | **Yes** — retry the quote with backoff; the same condition would also fail a payment, so do not route the call meanwhile. If it persists, quote `correlationId` |

...
When subscribers fetch an x402 token for a fiat plan, they pass `--payment-type fiat` (CLI) or `delegationConfig.providerPaymentMethodId = 'pm_...'` (SDK, Stripe/Braintree). The SDK auto-resolves `nvm:card-delegation` from the plan's `priceConfig`. For **Visa** plans, pass `delegationConfig.delegationId = '...'` — Visa delegations must be created in the Nevermined webapp (browser-only WebAuthn ceremony) and reused from the SDK by ID. Attempting `create_delegation(provider='visa', ...)` from the SDK is rejected with `BCK.VISA.0014` because the required `consumerPrompt` and `assuranceData` blobs can only be produced in-browser.
